...
- **오버-스무딩의 정의 (정의 1)**
    ...
    - 오버-스무딩은 적절한 노드-유사도 측정($\mu$)이 **층 수($n$)에 따라 기하급수적으로 $0$에 수렴하는 것**으로 공리적으로 정의됨. 즉, $\mu(X_n)$이 $C_1e^{-C_2n}$ 형태로 수렴함.
...
- **오버-스무딩 측정 방법**
    ...
    - **실험적 평가**: GCN, GAT, GraphSAGE와 같은 표준 GNN 아키텍처는 Texas, Cora, Cornell5 등 다양한 실제 그래프 데이터셋에서 모두 **층별 측정값(디리클레 에너지 및 MAD)이 층 수가 증가함에 따라 기하급수적으로 $0$에 수렴함**을 보임.
...
- **연속시간 GNN으로의 확장**
    ...
    - **연속시간 오버-스무딩 (정의 2)**: 노드 유사성 측정 $\mu$가 **시간($t$)에 따라 기하급수적으로 $0$에 수렴하는 것**으로 정의됨 (즉, $\mu(X(t))$가 $C_1e^{-C_2t}$ 형태로 수렴).
