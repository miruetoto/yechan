...
> (정의) 불편추정량의 정의는 아래와 같다. $$\hat{\boldsymbol\theta} \text{ is UB}\Leftrightarrow \forall {\boldsymbol \theta} \in {\bf \Theta}: \mathbb{E}_{\boldsymbol\theta}[\hat{\boldsymbol\theta}]=\boldsymbol\theta$$
...
> (정의) 최소분산불편추정량에 대한 정의는 아래와 같다. $$\hat{\boldsymbol\theta} \text{ is MVUE} \Leftrightarrow ~ (1) ~\hat{\boldsymbol\theta} \text{ is UB} ~~(2) ~\forall \hat{\boldsymbol \theta}^{UB}:\mathbb{V}_{\boldsymbol\theta}[\hat{\boldsymbol\theta}]\leq\mathbb{V}_{\boldsymbol\theta}[\hat{\boldsymbol\theta}^{UB}] $$ 여기에서 $\leq$ 는 positive semi-definite ordering을 의미한다. 
...
이 예제는 $(X_1,\dots,X_n)$ 의 결합확률분포에서 관찰한 샘플 $(x_1,\dots,x_n)$ 을 바탕으로 모수를 추정하는 기본 rationale을 살펴보자. 아래와 같은 상황을 가정하자.
...
즉 $X_1,X_2$의 결합확률분포는 $\theta$가 변함에 따라 같이 변화한다. 이를 이용해 우리는 $X_1,X_2$의 결합확률분포에서 관찰한 샘플들을 이용하여 $\theta$의 값을 역으로 추론한다. 정리하면 아래와 같다. 
