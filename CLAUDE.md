...
블로그 포스트 전체의 헤딩은 다음 위계로 통일:

`-` **본문 (callout 밖)**: `#` ~ `####` (h1 ~ h4) 까지만 사용.

`-` **Callout 내부**: `#####` ~ `######` (h5 ~ h6) 만 사용.
  - 기본은 `#####` (h5).
  - callout 안에서 더 작은 보조 헤딩이 필요한 경우만 `######` (h6).

`-` 이유 — callout 박스 안 내용은 본문의 보조 정보. `##`/`###` 같은 큰 헤딩은 callout 박스 안에서 시각적으로 산만하므로, callout 안에서는 본문 헤딩 수준 (`####` 까지) 보다 명백히 작게 (`#####` 부터).
...
::: {.callout-tip collapse="true" title="제목"}
