...
**📑 시리즈 인덱스**
- [시리즈 1편 — 5막 드라마](2026-04-30_TDL-trends-podcast.md)
- [시리즈 2편 — 이 동네 사람들](2026-04-30_TDL-cast-of-characters.md)
- **시리즈 3편 — 수학 무기고** ← 여기

---

**김K**: 지난 두 편 보고 또 댓글이 왔어요. "Dirac이니 Hodge니 Sheaf니, 그게 다 무슨 뜻인지 모르겠다."

**박P**: 좋은 질문이에요. 오늘은 **무기 해설편.** 9개 가져왔어요. 각 무기마다 — **뭐냐, 왜 쓰냐, 누가 쓰냐.**
...
**박P**: 가장 핫한 무기. 원래 **대수기하학**의 도구.

**김K**: 정의가 뭐죠?

**박P**: 그래프의 각 노드/엣지에 **벡터 공간(stalk)** 을 붙이고, 엣지를 따라 **선형 변환(restriction map)** 으로 정보를 옮기는 구조. 그래프 위에 "**국소 좌표계**"를 깐다고 보면 됩니다.

**김K**: 그냥 GCN보다 뭐가 좋은데요?

**박P**: GCN은 모든 엣지를 같은 방식으로 평균내요. **Sheaf는 엣지마다 다른 변환을 학습**합니다. 그래서 **heterophily(이웃 노드가 라벨이 다른 상황)** 에 강하고 **over-smoothing**도 덜 일어나요.
...
**박P**: 미분형식(differential form)을 세 부분으로 분해해요.
...
**박P**: **그래프 라플라시안 L = D - A**가 사실 **Hodge Laplacian의 0차 케이스**예요. 1차로 가면 **edge Laplacian**이 나오고, 거기서 위 분해가 정의됩니다. 즉 **edge signal에 대해서도 발산-회전 분해가 가능**해요.

**김K**: 응용은요?

**박P**: 본 데이터셋에서 **뇌 영상**이 압도적.
...
**박P**: 가장 새로 부상한 무기. 원래 **양자장론**에서 옴 (Paul Dirac).

**김K**: 양자장론?

**박P**: 네. **로런츠 다양체 위 스피너 장의 1차 미분 연산자**. 라플라시안 √Δ 같은 거. 그런데 이걸 **simplicial complex / hypergraph 위에서 이산화**할 수 있어요.

**김K**: 그게 왜 좋은데요?

**박P**: 결정적인 차이. **Hodge Laplacian은 한 차원 안에서만 작동**해요(노드면 노드, 엣지면 엣지). **Dirac은 차원을 가로질러 동작**합니다. 즉 노드 신호와 엣지 신호의 상호작용을 **하나의 연산자**로 다룰 수 있어요.

**김K**: 누가 미는데요?

**박P**: **Bianconi 그룹(Queen Mary)** 이 거의 단독으로 끌고 가는 분야.
...
**박P**: **GPT-2 분석** [12]. Harvard Med의 Asif Khan이 attention head를 token metric space의 Markov kernel로 보고, **레이어별 Ollivier-Ricci curvature를 측정.** 깊은 레이어로 갈수록 contractive support, H1 lifetime 감소.
...
**박P**: TDA(Topological Data Analysis)의 **간판 무기**. 가장 오래된 무기지만 여전히 강력.

**김K**: 한 줄 정의?

**박P**: 데이터에 **여러 스케일의 거리 임계값**을 줘가며, **homology(연결성, 구멍, 빈공간)** 가 어떻게 생기고 사라지는지 추적. **Birth-Death pair**로 나오는 게 persistence diagram.

**김K**: 단점은요?

**박P**: **계산이 무거워요.** 그래서 본 데이터셋에 **scalability 논문이 줄줄이** 나옴.
...
**박P**: 신호처리 출신 무기. **그래프 위 다중 스케일 분해.**

**김K**: 푸리에랑 뭐가 달라요?

**박P**: 푸리에는 **글로벌 진동수 분해**, framelet은 **지역적 + 다중 스케일 분해.** 즉 **time-frequency localization** 이 가능.

**김K**: 그래서 뭘 잘 잡아요?

**박P**: **High-frequency 성분.** 단순 GCN은 low-pass 필터라 heterophily에 약한데, framelet은 **high-pass 명시적 모델링**.
...
**박P**: **이산화된 외미분기하학.** 미분형식, 외미분 d, Hodge star ★를 simplicial mesh 위에서 정의.

**김K**: Hodge랑 같은 거 아니에요?

**박P**: 가까운 친척. **DEC가 더 일반적인 framework**. Hodge decomposition도 DEC의 한 부분으로 나옵니다. 그리고 DEC는 **3D 메시·점구름** 같은 기하 데이터에 자연스러워요.

**김K**: 본 데이터셋에서?

**박P**: **Sapienza의 TSP for 3D Point Cloud Data** [20]. **Cattai, Sardellitti, Barbarossa** — 점구름의 색상은 노드, 기하는 삼각형 중심에 정의된 edge signal로 모델링.

---

## ⚔️ 무기 8. **State-Space Models (Mamba, S4, S6)**

**박P**: 가장 최근 합류한 무기. **Transformer 대체 후보**.

**김K**: Mamba가 그래프에도 들어와요?

**박P**: 들어왔어요. **선형 시간 시퀀스 모델**의 위력을 **고차 데이터**에 붙이기 시작.

**김K**: 어떻게 묶어요?

**박P**: 본 데이터셋 대표작 [21] — **CCMamba: Selective State-Space Models for Higher-Order Graph Learning on Combinatorial Complexes**. Mamba의 selective scan을 **combinatorial complex** 위 message passing으로 일반화.

**김K**: 이거 시리즈 1편의 "트랜스포머 회귀론"하고 정확히 맞물리네요.

**박P**: 정확합니다. **Mozer의 DeepMind 논문**(state tracking 위상학적 한계)이 SSM에 명분을 주고, CCMamba가 실제 도구를 제공.

---

## ⚔️ 무기 9. **Optimal Transport / Wasserstein Distance**

**박P**: 두 분포 사이의 **수송 비용**을 거리로 보는 무기. 본 데이터셋에 OT가 많이 들어와요.
...
**박P**: **수학적 우아함 + 미분가능 구현(Sinkhorn)** 두 박자가 맞아서 그래요.
...
**김K**: 박사과정생이 자기 문제에 어떤 무기를 골라야 할지 한눈에 봤으면 좋겠어요.
...
**김K**: 이 표 한 장이면 이 분야 입문 박사한테 6개월치 로드맵 줄 수 있겠는데요.

**박P**: 맞아요. 다음 시리즈 4편(예정)에서는 — **아직 안 끝난 싸움들** 정리합시다.
