...
**G.** 동전 두 개가 있다고 해요. 하나는 앞면 60%, 하나는 앞면 40%. 그러면 $\alpha = (0.6,\, 0.4)$, $\beta = (0.4,\, 0.6)$.

**sup 정의로 계산해요.** $A = \{\text{앞면}\}$ 잡으면 $|\alpha(A) - \beta(A)| = |0.6 - 0.4| = 0.2$. $A = \{\text{뒷면}\}$ 도 같은 값. 그래서 $d_V = \sup_A |\alpha(A) - \beta(A)| = 0.2$.

**$\tfrac{1}{2}\sum$ 정의로 한 번 더 확인.** $\tfrac{1}{2}\bigl(|0.6-0.4| + |0.4-0.6|\bigr) = \tfrac{1}{2}(0.2+0.2) = 0.2$ — 같은 값.

두 정의의 동치성이 바로 다음에 볼 Lemma 4.1.2. 어쨌든 $d_V \in [0,1]$ 이고, $0$ 이면 같은 분포, $1$ 이면 완전 disjoint support.
