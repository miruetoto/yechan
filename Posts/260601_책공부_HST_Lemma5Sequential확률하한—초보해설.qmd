...
**Tower property 로 순차 결합.** conditioned step 을 시간 순으로 $s_1 < s_2 < \cdots < s_m$ ($m \leq K$) 이라 하자. 사건 $E$ 의 지시함수를 $\mathbf{1}\{E\}$ (성립 시 1, 아니면 0) 라 쓰면 $\mathcal{E} = \bigcap_{j=1}^{m}\{b'_{s_j}\leq\varepsilon_{s_j}\}$ 이고, 각 $\mathbf{1}\{b'_{s_j}\leq\varepsilon_{s_j}\}$ 는 $\mathcal{F}_{s_j}$-measurable 이므로 tower property 를 안쪽부터 적용한다:
...
$$\begin{aligned}
\prod_{s \in \{s\,:\, C_s = 1\}}\frac{\varepsilon_s}{b}
&\geq \prod_{s \in \{s\,:\, C_s = 1\}}\frac{\varepsilon_{\min}}{b} && (\because \varepsilon_s \geq \varepsilon_{\min}) \\
