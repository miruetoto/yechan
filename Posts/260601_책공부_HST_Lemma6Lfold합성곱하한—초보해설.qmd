...
여기서 $\mathrm{Leb}(\cdot)$ 은 Lebesgue 측도 (집합의 부피). 이제 $\mathrm{Leb}(P_z) > 0$ 을 보인다. $z\in\mathrm{int}(L\cdot\mathcal{O})$ 이므로 점 $(z/L,\ldots,z/L)\in\mathcal{O}^{L-1}$ 은 $P_z$ 의 점이고 ($z - (L-1)(z/L) = z/L\in\mathcal{O}$), $\mathcal{O}$ 가 열린집합이고 $z$ 가 내부점이라 이 점의 어떤 열린 근방 전체가 $P_z$ 에 포함된다. 따라서 $\mathrm{Leb}(P_z) > 0$, 즉
