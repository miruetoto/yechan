...
로 $(P^{\star})^{nL}(s^{\star},\cdot)\geq\eta\,\nu$ 가 $\mathsf{C}_0$ 위 균일하게 성립하고, $\mathsf{C}_0$ 는 small (따라서 petite) 이다.

여기서 $\nu$ 의 $\mathrm{Unif}(\mathsf{U})$ 는 **하한 measure** 이지 실제 도착 분포가 아님을 분명히 하자. 실제 전이분포는 $\mathsf{U}$ 위에서 비균등한 어떤 모양이고, 위 로그-막대들이 준 평평한 바닥 위에 얹혀 있을 뿐이다 ("$\geq$"). 바닥을 균등(공간적 상수)으로 그릴 수 있는 것은 밀도 하한 $\rho_{\mathbf{b}^{\flat}}(\mathbf{u})\geq c_*$ 가 $\mathsf{U}$ 의 **모든 점에서 같은 상수** $c_*$ 이기 때문이다 — 즉 "균등"은 밀도 하한의 균일성을 뜻하지 결과가 균등분포라는 뜻이 아니다 (그리고 이는 $\eta,\nu$ 가 모든 $s^{\star}$ 에 공통이라는 small-set 의 $s^{\star}$-균일성과는 별개의 층위다).

급소는 밀도다: 낙하 하중이 연속분포(Snowfall A)라서 $c_*>0$ 가 사는 것이고, 라운드 하중이 상수인 모델(Snowfall B)에서는 fall image 가 저차원 집합으로 붕괴하여 이 그림 전체가 성립하지 않는다.
