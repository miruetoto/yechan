...
을 보이는 것 (Lemma 19). 이전 단계까지 확보된 것은 "낙하 하중 $\mathbf{b}^{\sharp}\in[0,b]^{nL}$ 만으로 최종 높이차를 target $\mathsf{K}$ 어디로든 slice 부피 바닥 $m_0$ 을 갖고 보낼 수 있다"는 사실이고, Def 7 – Lem 19 가 채우는 것은 두 구멍 — walker 가 실제로 $v_n$ 에 도착하는가(경로는 높이차 부호가 결정), non-fall 하중의 흔들림이 이를 깨지 않는가 — 이다. 이 글은 그 여정을 다섯 장의 그림으로 걷는다. 준비:
