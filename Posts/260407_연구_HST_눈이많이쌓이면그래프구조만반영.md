...
| $\tau_n$ | round-skeleton 시각: $n$번째 $\mu_0$-draw가 일어나는 마르코프 체인 시각 |
...
> - **라운드 내 로컬**: $\tilde X_j$ — 라운드 $n$ (시작 시각 $\tau_n$, $\mu_0$-draw 시점) 분석에서 그 라운드의 $j$번째 스텝에서의 눈 위치 (Appendix B.3 이후로 사용). 전역 표기와의 관계는 $\tilde X_j := X_{\tau_n + j}$.
>
...
**Round의 정의.** **라운드(round)** $n$은 block 발생 후 새 눈이 $\tilde X_0 \sim \boldsymbol{\mu}_0$로 낙하하는 시점부터, flow chain을 거쳐 다음 block이 발생하는 시점까지이다. 한 라운드가 $m_n$ 스텝으로 구성되며 $2 \leq m_n \leq T_{max}+1$ (알고리즘 구조상). 라운드 $n$의 시작 시각을 $\tau_n$으로 쓴다 (= $n$번째 $\mu_0$-draw 시각).

이하 Step 2의 drift 분석은 **round-skeleton** $\{S^*_n := S(\tau_n)\}$ 위에서 1-step (= 1라운드) drift를 계산한다.
...
2. **Round-skeleton**: $\tau_n :=$ $n$번째 $\mu_0$-draw 시각 (= $n$번째 라운드 시작). $\{S(\tau_n)\}_{n\geq 0}$는 자체로 마르코프 체인 (라운드 시작은 $Z=0$의 fresh draw 직전 상태). 이 skeleton 위에서 1-step drift = "한 라운드 동안의 $\Phi$ 변화"를 분석한다.
...
**증명의 핵심 산출물**은 round-skeleton 1-step drift 부등식

$$\begin{aligned}
& \mathbb{E}[\Phi(\tau_{n+1}) - \Phi(\tau_n) \mid S(\tau_n)] \\
& \qquad \leq\; b\, M(\tau_n) \underbrace{\left[(T_{max}{+}1)\alpha - \tfrac{2p_0}{n-1}\right]}_{=:\, \Delta_{\mathrm{drift}}} + C_3
\end{aligned} \tag{$\star\star$}$$

이며, (DC) 아래에서 $(\star\star)$의 우변이 $M(\tau_n)$이 충분히 크면 음이 되고, Foster-Lyapunov로 Proposition 완성.
...
- **B.5** — 라운드 내내 $M$ 보존 ($M(\tau_n+s) \approx M(\tau_n)$)
- **B.6** — 결합 → $(\star\star)$ 도출
...
**(a) $\mathbb{E}[D^2 \mid S^*_n]$의 상한.** Lemma 1로 $|\Phi(s+1) - \Phi(s)| \leq 2b\,|\hbar(X_{s+1}, s)| + \tfrac{(n-1)b^2}{n} \leq 2bM(s) + b^2$. 한 라운드 $m_n \leq T_{max}+1 =: \Lambda/b + 1$ (여기서 $\Lambda := (T_{max}+1)b$) 스텝에 걸친 누적은 $|D| \leq 2(T_{max}+1)b\,M^* + (T_{max}+1)b^2$. $(\heartsuit)$로 $M^* \leq M(\tau_n) + \Lambda$이므로:

$$\mathbb{E}[D^2 \mid S^*_n] \;\leq\; O(b^2(T_{max}+1)^2\, M(\tau_n)^2) + O(1).$$

여기서 $D := \Phi(\tau_{n+1}) - \Phi(\tau_n)$ (한 라운드 동안의 $\Phi$ 변화).

**(b) $2\Phi(\tau_n)\cdot \mathbb{E}[D \mid S^*_n]$의 상한.** $(\star\star)$로 $\mathbb{E}[D \mid S^*_n] \leq bM(\tau_n) \Delta_{\mathrm{drift}} + C_3 = -c\,M(\tau_n) + C_3$ (여기서 $c := b\,|\Delta_{\mathrm{drift}}| > 0$). 또 항상 $\Phi \geq M^2/2$ (왜냐하면 $\hbar_{\max}^2 + \hbar_{\min}^2 \geq M^2/2$)이므로:

$$2\Phi(\tau_n) \cdot (-cM(\tau_n)) \;\leq\; -c\,M(\tau_n)^3.$$

한편 $2\Phi(\tau_n) \cdot C_3 \leq 2nC_3\,M(\tau_n)^2$ ($\Phi \leq nM^2$ 사용).

**(c) 결합.** Round-skeleton 위에서 $V$-drift 전체는:

$$\begin{aligned}
& \mathbb{E}[V(\tau_{n+1}) - V(\tau_n) \mid S^*_n] \\
& \qquad \leq\; \underbrace{O(b^2(T_{max}+1)^2)\,M(\tau_n)^2 + 2nC_3\,M(\tau_n)^2}_{=\,A_2\,M(\tau_n)^2} \;-\; c\,M(\tau_n)^3 \;+\; O(1)
\end{aligned}$$

$M(\tau_n)$이 충분히 크면 (정확히는 $M(\tau_n) > 2A_2/c$) $-cM^3$이 $A_2 M^2$를 압도하므로 **유한 집합 $\mathcal{B}_\Phi^\prime := \{M \leq 2A_2/c\}$ 바깥에서**

$$\mathbb{E}[V(\tau_{n+1}) - V(\tau_n) \mid S^*_n] \;\leq\; -\frac{c}{2}\,M(\tau_n)^3 + O(1).$$
...
Step 2의 Proposition을 증명한다. **Round-skeleton** $\{S^*_n := S(\tau_n)\}$ ($\tau_n$ = $n$번째 $\mu_0$-draw 시각) 위에서 한 라운드 동안의 Lyapunov 함수 $\Phi$의 drift를 분석하여 $(\star\star)$을 도출한 뒤, Foster-Lyapunov를 skeleton에 적용하고 원 체인으로 양의 재귀성을 전이하는 구성.
...
**Round-skeleton의 정의와 Markov 성질.** $\tau_n$을 $n$번째 $\mu_0$-draw가 일어나는 마르코프 체인 시각으로 정의하자 ($\tau_0$는 첫 라운드 시작). 라운드 길이 $m_n := \tau_{n+1} - \tau_n \in [2, T_{max}+1]$. **Skeleton chain** $\{S^*_n\}_{n \geq 0}$를 $S^*_n := S(\tau_n)$로 정의.

**$\{S^*_n\}$은 마르코프 체인.** $S^*_n = (\boldsymbol{\delta}(\tau_n), X_{\tau_n}, 0)$ ($Z(\tau_n) = 0$ 직전 상태). 라운드 $n+1$의 trajectory는 $S^*_n$에 의해서만 결정되는 random transition (μ_0 sampling + flow chain randomness) 의 함수이고, 다음 round-boundary 상태 $S^*_{n+1}$도 라운드 trajectory의 함수이므로, $S^*_{n+1}$ 분포는 $S^*_n$만으로 결정. 즉 strong Markov property에 의해 $\{S^*_n\}$도 자체로 Markov chain.

**핵심 이점: 부분 라운드가 없다.** 고정 시간 window $[t, t+T)$로 분석하면 양 끝에서 최대 2개의 부분 라운드(앞쪽 꼬리 + 뒤쪽 머리)가 발생하여 추가 baseline 항 (αt-무관 $T_{max}M(t)$ 보정)을 야기한다. 반면 round-skeleton의 1-step (= $\tau_n \to \tau_{n+1}$)은 정확히 한 완전 라운드와 일치하므로 **부분 라운드 보정 항이 원천 봉쇄**된다.

**1-라운드 동안의 $\Phi$ 변화.** $(\dagger)$의 per-step 등식에 **tower property** + telescoping을 라운드 길이 $m_n$ 만큼 적용:

$$\begin{aligned}
& \mathbb{E}[\Phi(\tau_{n+1}) - \Phi(\tau_n) \mid S^*_n] \\
& \qquad =\; 2b \sum_{j=0}^{m_n - 1} \mathbb{E}\big[\hbar(X_{\tau_n + j + 1},\, \tau_n + j) \mid S^*_n\big] + m_n \cdot \tfrac{(n-1)b^2}{n}
\end{aligned} \tag{$\dagger\dagger$}$$

여기서 $m_n$은 라운드 길이로 $S^*_n$ 조건부 확률변수 (단, $\leq T_{max}+1$ 결정적 상한).

**시간 스케일.** 라운드 내 시간을 $j \in \{0, 1, \ldots, m_n-1\}$로 매개화. 전역시각 $t' = \tau_n + j$. 라운드 시작 노드 $\tilde X_0 := X_{\tau_n}$ (fresh $\mu_0$-draw), $\tilde X_j := X_{\tau_n + j}$.

::: {.callout-tip collapse="true" title="($\\dagger\\dagger$) 직관: 한 라운드의 +b 축적 합산"}
($\dagger$)는 "1스텝에서 $\Phi$가 얼마나 변하는가"였다. ($\dagger\dagger$)는 그것을 한 라운드($m_n$ 스텝) 동안 합산한 것. 라운드 시작은 fresh $\mu_0$-draw이므로 $\hbar(\tilde X_0, \tau_n)$의 기댓값은 $\mu_0$-bias로 평가 가능 ((I) 분석). 라운드 안에서 단 한 번이라도 큰 낙차 사건이 발생하면 ((II) 분석), 그 음의 기여가 baseline 양의 기여를 압도한다.
:::

### B.3. 라운드별 기여 상한

라운드 $n$ (skeleton step $n$)이 $m_n$ 스텝으로 구성되고, 시작 시각 $\tau_n$에서 $\tilde X_0 := X_{\tau_n} \sim \boldsymbol{\mu}_0$ fresh draw로 시작한다. **이 라운드 내 $j$번째 스텝의 눈 위치**를 (전역 표기 $X_t$와 구별하여) 틸데 기호로

$$\tilde X_j \;:=\; X_{\tau_n + j}, \qquad j = 0, 1, \ldots, m_n - 1$$

로 쓴다. 즉 $\tilde X_j \in \{1,\ldots,n\}$이고, $j=0$은 라운드 시작 노드. 중심화 높이는 $\hbar(i, t) := h(v_i, t) - \bar{h}(t)$를 사용한다. 이하 표기 단순화를 위해 $\tau_n$ 대신 $t_r$로 쓰는 경우가 있는데 ($r$ = 라운드 인덱스), 동일한 의미이다.
...
라운드 $n$ (시작 시각 $\tau_n$, 길이 $m_n$) 의 시작 노드 $\tilde X_0 := X_{\tau_n} \sim \boldsymbol{\mu}_0$. 현재 높이 $\hbar(\cdot, \tau_n)$로부터 경사 간선 $(v_{i^\star}, v_{j^\star})$ 구성 (비둘기집으로 높이차 $\geq M(\tau_n)/(n-1)$). 경사 간선은 **라운드마다 달라질 수 있지만** 연결성으로 항상 존재.

**큰 낙차 사건 $E_n$**: 눈이 $v_{i^\star}$에 떨어지고 ($\tilde X_0 = i^\star$), 첫 flow 스텝에서 $v_{j^\star}$로 이동.

확률은 (경사 간선 선택과 무관하게) **그래프-uniform 하한** 가짐:

$$\Pr(E_n \mid S^*_n) \;\geq\; \mu_{min} \cdot P_{RW,\min} \;=:\; p_0 \;>\; 0$$
...
큰 낙차 라운드의 flow 개선: $E_n$ 발생 시 스텝 $j=0$에서 $\tilde X_0 = i^\star$, 스텝 $j=1$에서 $\tilde X_1 = j^\star$이다. 이때 $j^\star$에는 스텝 1에서 $+b$가 쌓이므로 평균 $\bar{h}(\cdot)$의 $b/n$ 상승분을 빼주면

$$\hbar(\tilde X_1, \tau_n{+}1) = \hbar(j^\star, \tau_n) + b\cdot\frac{n-1}{n} < \hbar(j^\star, \tau_n) + b.$$

이후 $j \geq 2$에 대해서는 B.3과 동일한 체이닝으로 $\hbar(\tilde X_j, \tau_n{+}j) < \hbar(\tilde X_1, \tau_n{+}1) + (j-1)b$가 성립하므로, $j \geq 1$에서 통일하여:

$$\hbar(\tilde X_j, \tau_n{+}j) < \hbar(j^\star, \tau_n) + jb \;\;\overset{(\ast)}{\leq}\;\; \hbar(i^\star, \tau_n) - \frac{M(\tau_n)}{n-1} + jb.$$

(마지막 부등호 $(\ast)$는 경사 간선의 정의 $\hbar(j^\star, \tau_n) \leq \hbar(i^\star, \tau_n) - M(\tau_n)/(n-1)$을 대입한 것이다.)

$\tilde X_0 = i^\star$이므로 각 항은 $\hbar(\tilde X_j, \tau_n{+}j) - \hbar(\tilde X_0, \tau_n) \leq -M(\tau_n)/(n-1) + jb$. $j=1$부터 $m_n-1$까지 합하면:

$$\sum_{j=1}^{m_n-1}\big(\hbar(\tilde X_j, \tau_n{+}j) - \hbar(\tilde X_0, \tau_n)\big) \leq -(m_n-1)\frac{M(\tau_n)}{n-1} + b\cdot\frac{(m_n-1)m_n}{2}$$

$m_n \leq T_{max}+1$이므로 $b\cdot\frac{(m_n-1)m_n}{2} \leq C_{round}$이고:

$$\sum_{j=1}^{m_n-1}\big(\hbar(\tilde X_j, \tau_n{+}j) - \hbar(\tilde X_0, \tau_n)\big) \leq -(m_n-1)\frac{M(\tau_n)}{n-1} + C_{round} \tag{$\spadesuit$}$$

큰 낙차 없는 라운드 대비, **개선량**은 최소 $M(\tau_n)/(n-1)$ ($m_n \geq 2$이므로).

::: {.callout-tip collapse="true" title="두 합의 직관적 의미"}
- $\sum_{j=0}^{m_n-1} \hbar(\tilde X_j, \tau_n{+}j)$는 라운드 $n$ 동안 walker가 방문한 노드들의 centered height 총합이다. 이 값이 클수록 높은 곳을 많이 방문한 것이고, $\Phi$의 증가에 기여한다.
- $\sum_{j=1}^{m_n-1} \big(\hbar(\tilde X_j, \tau_n{+}j) - \hbar(\tilde X_0, \tau_n)\big)$는 출발점 대비 얼마나 낮은 곳으로 내려갔는지를 측정한다. 큰 낙차 사건에서는 walker가 $v_{i^\star}$(높은 곳)에서 출발하여 $v_{j^\star}$(낮은 곳)로 떨어지므로 이 합이 크게 음수가 되고, 이것이 $\Phi$를 줄이는 핵심 메커니즘이다.
:::

### B.5. 1-라운드 내내 $M$ 보존

B.4의 ($\spadesuit$) 개선량 부등식은 라운드 시작 시점 $M(\tau_n)$에 비례한다. 라운드 길이 $m_n \leq T_{max}+1$로 결정적 상한이 있으므로, 라운드 내내 $M$이 크게 변하지 않음을 보이는 게 쉽다 (window 분석 때보다 훨씬 간단).

**관찰 (한 스텝당 $M$의 변화량 $\leq b$).** 한 스텝에 노드 하나에만 $+b$가 쌓이므로, 중심화 높이 $\hbar$의 변화는 해당 노드 $\hbar \mapsto \hbar + b(n{-}1)/n$, 나머지 $\hbar \mapsto \hbar - b/n$이다. 즉 $M = \max\hbar - \min\hbar$의 한 스텝 변동은 절댓값으로 $\leq b$. 따라서 라운드 $n$ 안의 임의 시각 $\tau_n + j$ ($0 \leq j \leq m_n \leq T_{max}+1$)에 대해

$$|M(\tau_n + j) - M(\tau_n)| \;\leq\; (T_{max}+1)\,b \;=:\; \Lambda. \tag{$\heartsuit$}$$

**가정 $M(\tau_n) > 2\Lambda$로부터의 결론.** $(\heartsuit)$에 의해 라운드 내내

$$M(\tau_n + j) \;>\; M(\tau_n) - \Lambda \;>\; 2\Lambda - \Lambda \;=\; \Lambda \;=\; \tfrac{1}{2}M(\tau_n).$$

즉 **$M$이 라운드 내내 반토막은 나지 않는다.** 따라서 ($\spadesuit$)의 개선량을 $M(\tau_n)$ 만으로 균일하게 하한 가능.

::: {.callout-tip collapse="true" title="왜 하필 $2\\Lambda$? 그리고 $M(\\tau_n) \\leq 2\\Lambda$ 영역은 어떻게 처리하나?"}

**① 왜 $2\Lambda$인가? — 반토막 방지용 여유분**

라운드 내내 $M$이 크게 줄지 않게 하려는 것. $M$은 한 스텝에 $\leq b$, 라운드 길이 $\leq T_{max}+1$이므로 총 $\leq \Lambda$만큼 변동 가능. $M(\tau_n) > 2\Lambda$로 잡으면, 최악의 감소를 겪어도 $M(\tau_n) - \Lambda > \Lambda = M(\tau_n)/2$로 반토막은 면한다.

**② $M(\tau_n) \leq 2\Lambda$ 영역 — 그냥 refuge에 흡수**
...
$(\dagger\dagger)$로부터 1-라운드 동안의 $\Phi$ 변화는

$$\mathbb{E}[\Phi(\tau_{n+1}) - \Phi(\tau_n) \mid S^*_n] \;=\; 2b\, \mathbb{E}\bigg[\sum_{j=0}^{m_n - 1} \hbar(X_{\tau_n + j + 1}, \tau_n + j)\,\Big|\,S^*_n\bigg] + \mathbb{E}[m_n \mid S^*_n]\cdot \tfrac{(n-1)b^2}{n}$$

이고, 라운드 길이 $m_n \leq T_{max}+1$ 결정적 상한이므로 마지막 항 $\leq (T_{max}+1)\,b^2(n-1)/n$ 상수.

$\sum_{j=0}^{m_n-1}\hbar(\tilde X_j, \tau_n+j) = m_n \hbar(\tilde X_0, \tau_n) + \sum_{j=1}^{m_n-1}(\hbar(\tilde X_j) - \hbar(\tilde X_0))$로 분해해서 (I) + (II) 처리.

**(I) $\mu_0$-편향 baseline.** $\tilde X_0 = X_{\tau_n} \sim \boldsymbol{\mu}_0$ (fresh draw)이므로 그 centered height의 평균은 $\boldsymbol{\mu}_0$ 가중치로 본 $\hbar(\cdot, \tau_n)$의 평균:

$$\mathbb{E}[\hbar(\tilde X_0, \tau_n) \mid S^*_n] = \sum_i \mu_0(i)\,\hbar(i, \tau_n)$$

**기준점 (정칙 그래프).** 만약 $\boldsymbol{\mu}_0$가 **균등분포** $\mathbf{u} = (1/n,\ldots,1/n)$이면

$$\sum_i \tfrac{1}{n}\,\hbar(i, \tau_n) = \tfrac{1}{n}\underbrace{\sum_i \hbar(i, \tau_n)}_{=\,0} = 0$$

즉 **낙하 편향에 의한 $\Phi$ drift 기여가 없다.** 이게 정칙 그래프에서 Theorem이 자동 성립하는 이유.
...
$$\mathbb{E}[\hbar(\tilde X_0, \tau_n) \mid S^*_n] = \sum_i \big(\mu_0(i) - \tfrac{1}{n}\big)\,\hbar(i, \tau_n)$$

최악의 경우 (초과 $\alpha/2$를 max에, 부족 $\alpha/2$를 min에서 차감):

$$\boxed{\mathbb{E}[\hbar(\tilde X_0, \tau_n) \mid S^*_n] \;\leq\; \frac{\alpha}{2}\,M(\tau_n)}$$

따라서 (I) baseline 합산 ($m_n \leq T_{max}+1$):

$$\begin{aligned}
\text{(I)} \;:=\;\; & 2b\, \mathbb{E}[m_n \cdot \hbar(\tilde X_0, \tau_n) \mid S^*_n] \\
\leq\;\; & 2b \cdot (T_{max}+1) \cdot \frac{\alpha M(\tau_n)}{2} \\
=\;\; & b\,(T_{max}+1)\,\alpha\,M(\tau_n)
\end{aligned}$$

(엄밀히는 $m_n$과 $\hbar(\tilde X_0)$이 같은 라운드 안에서 약간 의존성을 가질 수 있으나, $m_n \leq T_{max}+1$ 결정적 상한과 $\hbar(\tilde X_0)$ 기댓값 상한 $\alpha M/2$를 곱해 worst-case 결합한 것.)

**(II) 큰 낙차 개선.** 라운드 $n$에서 big-drop 사건 $E_n$이 발생하면 ($\Pr(E_n \mid S^*_n) \geq p_0$), B.4의 ($\spadesuit$) 부등식에서 $m_n \geq 2$이므로 $-(m_n - 1) \leq -1$:

$$\sum_{j=1}^{m_n-1}\big(\hbar(\tilde X_j) - \hbar(\tilde X_0)\big) \cdot \mathbf{1}_{E_n} \;\leq\; \left[-\frac{M(\tau_n)}{n-1} + C_{round}\right] \mathbf{1}_{E_n}$$
...
$$\begin{aligned}
\text{(II)} \;=\;\; & 2b\, \mathbb{E}\Big[\sum_{j=1}^{m_n-1}(\hbar(\tilde X_j) - \hbar(\tilde X_0))\,\Big|\,S^*_n\Big] \\
\leq\;\; & 2b \cdot \Pr(E_n \mid S^*_n) \cdot \left[-\frac{M(\tau_n)}{n-1} + C_{round}\right] + 2b\,(1-\Pr(E_n))\,C_{round} \\
\leq\;\; & -\frac{2bp_0\,M(\tau_n)}{n-1} + 2b\,C_{round}
\end{aligned}$$

(중간 단계에서 $\Pr(E_n) \geq p_0$이므로 음의 항이 $\geq -2bp_0 M/(n-1)$ — 부등호 방향 주의.)
...
$$C_3 \;:=\; 2b\,C_{round} \;+\; (T_{max}+1)\cdot \tfrac{(n-1)b^2}{n}$$

는 $M(\tau_n)$에 무관하고 $\mathcal{G},\,b,\,T_{max}$에만 의존하는 **결정적** 상수. (부분 라운드 보정 항 $4T_{max}$가 사라진 것에 주목 — round-skeleton 덕분.)

(B.6 종료.)

### B.7. Drift 조건 $\Delta_{\mathrm{drift}} < 0$의 충분 조건

Round-skeleton 1-step drift $(\star\star)$의 우변이 $M(\tau_n)$이 커질 때 확실히 음이 되려면:
...
이 조건이 성립하면, $|\hbar(i, \tau_n)| \leq M(\tau_n)$로부터 $\Phi(\tau_n) \leq n\,M(\tau_n)^2$, 즉 $M(\tau_n)^2 \geq \Phi(\tau_n)/n$. 따라서 $\Phi(\tau_n) > C_1 := n\big(\tfrac{C_3}{|\Delta_{\mathrm{drift}}|\, b}\big)^2$이면 $M(\tau_n) > C_3/(|\Delta_{\mathrm{drift}}| b)$이고 $(\star\star)$의 우변이 음이 된다.
...
**(i) Round-skeleton에 적용.** $(\star\star\star)$이 성립하면 $(\star\star)$의 drift를 다음과 같이 정규화 가능: $M(\tau_n) > C_3/(b|\Delta_{\mathrm{drift}}|) + 1/(b|\Delta_{\mathrm{drift}}|)$이면 $(\star\star)$의 우변 $\leq -1$. 즉

$$\mathcal{B}_\Phi^* \;:=\; \{S^* = (\boldsymbol{\delta}, X, 0) : \Phi(\boldsymbol{\delta}) \leq C_1\}$$

로 놓으면 ($C_1$은 위 임계값 이하의 $\Phi$ 영역) skeleton chain $\{S^*_n\}$이 $\mathcal{B}_\Phi^*$ 바깥에서 1-step drift $\leq -1$ 만족. 

**$\mathcal{B}_\Phi^*$의 유한성.** $\boldsymbol{\delta} \in \boldsymbol{\delta}(0) + b\mathbb{Z}^{n-1}$ 격자 + $\Phi$ coercive ⇒ $\{\Phi \leq C_1\}$ 격자 유한. $X \in \{1,\ldots,n\}$, $Z = 0$ (round-boundary) 고정. 따라서 $\mathcal{B}_\Phi^*$ 유한.

Foster-Lyapunov 적용으로 skeleton 체인 양의 재귀: $\mathbb{E}[\tau^*_{\mathcal{B}^*_\Phi}] < \infty$.

**(ii) Skeleton ↔ 원 체인 전이.** 라운드 길이 $m_n \leq T_{max}+1$ 결정적 상한이므로

$$\tau_{n+1} - \tau_n \leq T_{max}+1.$$

원 체인이 $\mathcal{B}^*_\Phi$ ($\subseteq \mathcal{X}$로 자연스럽게 포함, 단 round-boundary 한정) 또는 그것을 "fattened"한 집합 $\mathcal{B}_\Phi := \{S : \Phi(\boldsymbol{\delta}(S)) \leq C_1\}$ (모든 $X, Z$ 허용) 으로 복귀하는 시각 $\tau_{\mathcal{B}_\Phi}$는

$$\tau_{\mathcal{B}_\Phi} \;\leq\; \tau_{T_n} + \text{(skeleton step }n\text{ 까지의 누적 시간)} \;\leq\; (T_{max}+1) \cdot \tau^*_{\mathcal{B}^*_\Phi}$$

따라서 $\mathbb{E}[\tau_{\mathcal{B}_\Phi}] \leq (T_{max}+1)\,\mathbb{E}[\tau^*_{\mathcal{B}^*_\Phi}] < \infty$. 즉 **원 체인도 양의 재귀**.

**$\mathcal{B}_\Phi$의 유한성 (원 체인용).** (i) $\boldsymbol{\delta}$ 격자 ∩ coercive $\Phi$ 사블레벨 = 유한, (ii) $X \in \{1,\ldots,n\}$, $Z \in \{0,\ldots,T_{max}\}$ 유한. 따라서 $\mathcal{B}_\Phi$ 유한 집합.
