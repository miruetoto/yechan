...
> - **라운드 내 로컬**: $\tilde X_j$ — 라운드 $n$ (시작 시각 $t_r$, $\mu_0$-draw 시점) 분석에서 그 라운드의 $j$번째 스텝에서의 눈 위치 (Appendix B.3 이후로 사용). 전역 표기와의 관계는 $\tilde X_j := X_{t_r + j}$.
>
...
이하 Step 2의 drift 분석은 **round-skeleton** $\{S^*_r := S(t_r)\}$ 위에서 1-step (= 1라운드) drift를 계산한다.
...
2. **Round-skeleton**: $t_r :=$ $n$번째 $\mu_0$-draw 시각 (= $n$번째 라운드 시작). $\{S(t_r)\}_{n\geq 0}$는 자체로 마르코프 체인 (라운드 시작은 $Z=0$의 fresh draw 직전 상태). 이 skeleton 위에서 1-step drift = "한 라운드 동안의 $\Phi$ 변화"를 분석한다.
...
**(a) $\mathbb{E}[D^2 \mid S^*_r]$의 상한.** Lemma 1로 $|\Phi(s+1) - \Phi(s)| \leq 2b\,|\hbar(X_{s+1}, s)| + \tfrac{(n-1)b^2}{n} \leq 2bM(s) + b^2$. 한 라운드 $m_r \leq T_{max}+1 =: \Lambda/b + 1$ (여기서 $\Lambda := (T_{max}+1)b$) 스텝에 걸친 누적은 $|D| \leq 2(T_{max}+1)b\,M^* + (T_{max}+1)b^2$. $(\heartsuit)$로 $M^* \leq M(t_r) + \Lambda$이므로:

$$\mathbb{E}[D^2 \mid S^*_r] \;\leq\; O(b^2(T_{max}+1)^2\, M(t_r)^2) + O(1).$$

여기서 $D := \Phi(t_{r+1}) - \Phi(t_r)$ (한 라운드 동안의 $\Phi$ 변화).

**(b) $2\Phi(t_r)\cdot \mathbb{E}[D \mid S^*_r]$의 상한.** $(\star\star)$로 $\mathbb{E}[D \mid S^*_r] \leq bM(t_r) \Delta_{\mathrm{drift}} + C_3 = -c\,M(t_r) + C_3$ (여기서 $c := b\,|\Delta_{\mathrm{drift}}| > 0$). 또 항상 $\Phi \geq M^2/2$ (왜냐하면 $\hbar_{\max}^2 + \hbar_{\min}^2 \geq M^2/2$)이므로:

$$2\Phi(t_r) \cdot (-cM(t_r)) \;\leq\; -c\,M(t_r)^3.$$

한편 $2\Phi(t_r) \cdot C_3 \leq 2nC_3\,M(t_r)^2$ ($\Phi \leq nM^2$ 사용).

**(c) 결합.** Round-skeleton 위에서 $V$-drift 전체는:

$$\begin{aligned}
& \mathbb{E}[V(t_{r+1}) - V(t_r) \mid S^*_r] \\
& \qquad \leq\; \underbrace{O(b^2(T_{max}+1)^2)\,M(t_r)^2 + 2nC_3\,M(t_r)^2}_{=\,A_2\,M(t_r)^2} \;-\; c\,M(t_r)^3 \;+\; O(1)
\end{aligned}$$

$M(t_r)$이 충분히 크면 (정확히는 $M(t_r) > 2A_2/c$) $-cM^3$이 $A_2 M^2$를 압도하므로 **유한 집합 $\mathcal{B}_\Phi^\prime := \{M \leq 2A_2/c\}$ 바깥에서**

$$\mathbb{E}[V(t_{r+1}) - V(t_r) \mid S^*_r] \;\leq\; -\frac{c}{2}\,M(t_r)^3 + O(1).$$
...
Step 2의 Proposition을 증명한다. **Round-skeleton** $\{S^*_r := S(t_r)\}$ ($t_r$ = $n$번째 $\mu_0$-draw 시각) 위에서 한 라운드 동안의 Lyapunov 함수 $\Phi$의 drift를 분석하여 $(\star\star)$을 도출한 뒤, Foster-Lyapunov를 skeleton에 적용하고 원 체인으로 양의 재귀성을 전이하는 구성.
...
**Round-skeleton의 정의와 Markov 성질.** $t_r$을 $r$번째 $\mu_0$-draw가 일어나는 마르코프 체인 시각으로 정의하자 ($t_0$는 첫 라운드 시작). 라운드 길이 $m_r := t_{r+1} - t_r \in [2, T_{max}+1]$. **Skeleton chain** $\{S^*_r\}_{r \geq 0}$를 $S^*_r := S(t_r)$로 정의.

**$\{S^*_r\}$은 마르코프 체인.** $S^*_r = (\boldsymbol{\delta}(t_r), X_{t_r}, 0)$ ($Z(t_r) = 0$ 직전 상태). 라운드 $r+1$의 trajectory는 $S^*_r$에 의해서만 결정되는 random transition (μ_0 sampling + flow chain randomness) 의 함수이고, 다음 round-boundary 상태 $S^*_{r+1}$도 라운드 trajectory의 함수이므로, $S^*_{r+1}$ 분포는 $S^*_r$만으로 결정. 즉 strong Markov property에 의해 $\{S^*_r\}$도 자체로 Markov chain.

**핵심 이점: 부분 라운드가 없다.** 고정 시간 window $[t, t+T)$로 분석하면 양 끝에서 최대 2개의 부분 라운드(앞쪽 꼬리 + 뒤쪽 머리)가 발생하여 추가 baseline 항 (αt-무관 $T_{max}M(t)$ 보정)을 야기한다. 반면 round-skeleton의 1-step (= $t_r \to t_{r+1}$)은 정확히 한 완전 라운드와 일치하므로 **부분 라운드 보정 항이 원천 봉쇄**된다.

**1-라운드 동안의 $\Phi$ 변화.** $(\dagger)$의 per-step 등식에 **tower property** + telescoping을 라운드 길이 $m_r$ 만큼 적용:

$$\begin{aligned}
& \mathbb{E}[\Phi(t_{r+1}) - \Phi(t_r) \mid S^*_r] \\
& \qquad =\; 2b \sum_{j=0}^{m_r - 1} \mathbb{E}\big[\hbar(X_{t_r + j + 1},\, t_r + j) \mid S^*_r\big] + m_r \cdot \tfrac{(n-1)b^2}{n}
\end{aligned} \tag{$\dagger\dagger$}$$

여기서 $m_r$은 라운드 길이로 $S^*_r$ 조건부 확률변수 (단, $\leq T_{max}+1$ 결정적 상한).
...
($\dagger$)는 "1스텝에서 $\Phi$가 얼마나 변하는가"였다. ($\dagger\dagger$)는 그것을 한 라운드($m_r$ 스텝) 동안 합산한 것. 라운드 시작은 fresh $\mu_0$-draw이므로 $\hbar(\tilde X_0, t_r)$의 기댓값은 $\mu_0$-bias로 평가 가능 ((I) 분석). 라운드 안에서 단 한 번이라도 큰 낙차 사건이 발생하면 ((II) 분석), 그 음의 기여가 baseline 양의 기여를 압도한다.
:::

### B.3. 라운드별 기여 상한

라운드 $n$ (skeleton step $n$)이 $m_r$ 스텝으로 구성되고, 시작 시각 $t_r$에서 $\tilde X_0 := X_{t_r} \sim \boldsymbol{\mu}_0$ fresh draw로 시작한다. **이 라운드 내 $j$번째 스텝의 눈 위치**를 (전역 표기 $X_t$와 구별하여) 틸데 기호로
...
$$\Pr(E_r \mid S^*_r) \;\geq\; \mu_{min} \cdot P_{RW,\min} \;=:\; p_0 \;>\; 0$$
...
::: {.callout-tip collapse="true" title="왜 하필 $2\\Lambda$? 그리고 $M(\t_r) \\leq 2\\Lambda$ 영역은 어떻게 처리하나?"}
...
$$\mathbb{E}[\Phi(t_{r+1}) - \Phi(t_r) \mid S^*_r] \;=\; 2b\, \mathbb{E}\bigg[\sum_{j=0}^{m_r - 1} \hbar(X_{t_r + j + 1}, t_r + j)\,\Big|\,S^*_r\bigg] + \mathbb{E}[m_r \mid S^*_r]\cdot \tfrac{(n-1)b^2}{n}$$

이고, 라운드 길이 $m_r \leq T_{max}+1$ 결정적 상한이므로 마지막 항 $\leq (T_{max}+1)\,b^2(n-1)/n$ 상수.

$\sum_{j=0}^{m_r-1}\hbar(\tilde X_j, t_r+j) = m_r \hbar(\tilde X_0, t_r) + \sum_{j=1}^{m_r-1}(\hbar(\tilde X_j) - \hbar(\tilde X_0))$로 분해해서 (I) + (II) 처리.

**(I) $\mu_0$-편향 baseline.** $\tilde X_0 = X_{t_r} \sim \boldsymbol{\mu}_0$ (fresh draw)이므로 그 centered height의 평균은 $\boldsymbol{\mu}_0$ 가중치로 본 $\hbar(\cdot, t_r)$의 평균:

$$\mathbb{E}[\hbar(\tilde X_0, t_r) \mid S^*_r] = \sum_i \mu_0(i)\,\hbar(i, t_r)$$
...
$$\mathbb{E}[\hbar(\tilde X_0, t_r) \mid S^*_r] = \sum_i \big(\mu_0(i) - \tfrac{1}{n}\big)\,\hbar(i, t_r)$$

최악의 경우 (초과 $\alpha/2$를 max에, 부족 $\alpha/2$를 min에서 차감):

$$\boxed{\mathbb{E}[\hbar(\tilde X_0, t_r) \mid S^*_r] \;\leq\; \frac{\alpha}{2}\,M(t_r)}$$

따라서 (I) baseline 합산 ($m_r \leq T_{max}+1$):

$$\begin{aligned}
\text{(I)} \;:=\;\; & 2b\, \mathbb{E}[m_r \cdot \hbar(\tilde X_0, t_r) \mid S^*_r] \\
\leq\;\; & 2b \cdot (T_{max}+1) \cdot \frac{\alpha M(t_r)}{2} \\
=\;\; & b\,(T_{max}+1)\,\alpha\,M(t_r)
\end{aligned}$$

(엄밀히는 $m_r$과 $\hbar(\tilde X_0)$이 같은 라운드 안에서 약간 의존성을 가질 수 있으나, $m_r \leq T_{max}+1$ 결정적 상한과 $\hbar(\tilde X_0)$ 기댓값 상한 $\alpha M/2$를 곱해 worst-case 결합한 것.)

**(II) 큰 낙차 개선.** 라운드 $n$에서 big-drop 사건 $E_r$이 발생하면 ($\Pr(E_r \mid S^*_r) \geq p_0$), B.4의 ($\spadesuit$) 부등식에서 $m_r \geq 2$이므로 $-(m_r - 1) \leq -1$:
...
$$\begin{aligned}
\text{(II)} \;=\;\; & 2b\, \mathbb{E}\Big[\sum_{j=1}^{m_r-1}(\hbar(\tilde X_j) - \hbar(\tilde X_0))\,\Big|\,S^*_r\Big] \\
\leq\;\; & 2b \cdot \Pr(E_r \mid S^*_r) \cdot \left[-\frac{M(t_r)}{n-1} + C_{round}\right] + 2b\,(1-\Pr(E_r))\,C_{round} \\
...
로 놓으면 ($C_1$은 위 임계값 이하의 $\Phi$ 영역) skeleton chain $\{S^*_r\}$이 $\mathcal{B}_\Phi^*$ 바깥에서 1-step drift $\leq -1$ 만족. 
...
**(ii) Skeleton ↔ 원 체인 전이.** 라운드 길이 $m_r \leq T_{max}+1$ 결정적 상한이므로

$$t_{r+1} - t_r \leq T_{max}+1.$$
...
$$\tau_{\mathcal{B}_\Phi} \;\leq\; (T_{max}+1) \cdot \tau^*_{\mathcal{B}^*_\Phi}$$ 

(skeleton이 $r$번째 step에 $\mathcal{B}^*_\Phi$에 도달했다면 원 체인은 늦어도 $t_r \leq r(T_{max}+1)$ 시점에 도달)
