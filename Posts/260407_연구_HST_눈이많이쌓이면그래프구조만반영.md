...
| 기호 | 의미 |
|------|------|
| $X_{fall} \in \{1,\ldots,n\}$ | 눈의 초기 낙하 노드 인덱스 (flow 전) |
...
Lemma 1에서 $v_\ell$은 "눈이 실제로 쌓이는 노드"였다. HST에서 매 스텝마다 현재 눈 위치 $X(t{+}1)$에 $+b$가 쌓이므로, Lemma 1의 $\hbar(\ell, t)$를 $\hbar(X(t{+}1), t)$로 바꾸고 기대값을 씌운 것이 ($\dagger$)이다.
...
> **Figure.** $v^+$에서 $v^-$로의 경로 위에서 비둘기집 원리에 의해 높이차가 $M(t)/(n-1)$ 이상인 경사 간선 $(v_{i^\star}, v_{j^\star})$가 존재한다. 눈이 $v_{i^\star}$에 떨어지면($\mu_0(v_{i^\star})$ 확률) flow를 통해 $v_{j^\star}$ 이하로 내려간다.

$\mathcal{G}$가 연결이므로, 최고점 $v^+ = \arg\max_i \hbar(i, t)$에서 최저점 $v^- = \arg\min_i \hbar(i, t)$로의 경로 $v^+ = v'_0, v'_1, \ldots, v'_m = v^-$ ($m \leq n-1$)가 존재한다. 경로를 따른 총 높이 낙차는:

$$M(t) = \hbar(v'_0, t) - \hbar(v'_m, t) = \sum_{i=0}^{m-1}\big(\hbar(v'_i, t) - \hbar(v'_{i{+}1}, t)\big)$$

위 합에는 $m$개의 항이 있고, 각 항 $\hbar(v'_i, t) - \hbar(v'_{i{+}1}, t)$는 경로 위 인접 노드 간의 높이 낙차이다. 이 $m$개 항의 **평균**은 $M(t)/m \geq M(t)/(n-1)$이다. 평균이 $M(t)/(n-1)$ 이상이므로, 비둘기집 원리(pigeonhole principle)에 의해 **평균 이상의 낙차를 가진 간선이 최소 하나 존재**한다:

$$\exists\, i \quad \text{s.t.} \quad \hbar(v'_i, t) - \hbar(v'_{i{+}1}, t) \geq \frac{M(t)}{n-1}$$

이 간선 $(v'_i, v'_{i+1})$을 **경사 간선(steep edge)** $(v_{i^\star}, v_{j^\star})$이라 하자 ($\hbar(i^\star,t) > \hbar(j^\star,t)$).
...
**Lemma 1a ($T$-step Drift).** $T := k(T_{max}+1)$, $M(t) := \max_i \hbar(i, t) - \min_i \hbar(i, t)$라 할 때, 양의 상수 $\gamma = \gamma(\mathcal{G}) > 0$이 존재하여: $M(t)$가 충분히 크면,
...
라운드 $r$이 $m_r$스텝이고, $X_{fall}(r) \sim \boldsymbol{\mu}_0$에서 시작한다. 라운드의 첫 스텝을 시각 $t_0$라 하고, 스텝 $j$에서 방문하는 노드 인덱스를 $X_j \in \{1,\ldots,n\}$라 쓰자. 중심화 높이 $\hbar(i, t) := h(v_i, t) - \bar{h}(t)$를 사용한다.
...
그렇다면 올바른 상한은 무엇인가? Flow의 정의에 의해 $h(v_{X_j}, t_0{+}j) \leq h(v_{X_{j-1}}, t_0{+}j{-}1) + b$이므로, 체이닝하면:

$$h(v_{X_j}, t_0{+}j) \leq h(v_{X_{j-1}}, t_0{+}j{-}1) + b \leq \cdots \leq h(v_{X_0}, t_0) + jb$$

한편, $\hbar(i, t) = h(v_i, t) - \bar{h}(t)$이고 $\bar{h}(t_0{+}j) = \bar{h}(t_0) + jb/n$이므로:

$$\hbar(X_j, t_0{+}j) - \hbar(X_0, t_0) = \big[h(v_{X_j}, t_0{+}j) - h(v_{X_0}, t_0)\big] - \frac{jb}{n} \leq jb - \frac{jb}{n} = jb\cdot\frac{n-1}{n} < jb$$
...
$$\text{(라운드 합)}_r := \sum_{j=0}^{m_r - 1} \hbar(X_j, t_0{+}j) < \sum_{j=0}^{m_r-1}\big(\hbar(X_0, t_0) + jb\big) = m_r \cdot \hbar(X_0, t_0) + b\sum_{j=0}^{m_r-1} j$$

$\sum_{j=0}^{m_r-1} j = \frac{(m_r-1)m_r}{2}$이고, $m_r \leq T_{max}$이므로:

$$\text{(라운드 합)}_r < m_r \cdot \hbar(X_0, t_0) + C_{round}$$

여기서 $C_{round} := \frac{T_{max}(T_{max}+1)}{2}\,b$는 그래프에만 의존하는 상수이다.

**Step B: 큰 낙차 사건(Big Drop Event)의 확률과 기여.**

라운드 $r$에서 "큰 낙차 사건" $E_r$: 눈이 경사 간선의 높은 끝 $v_{i^\star}$에 떨어지고 ($X_{fall} = i^\star$), 첫 flow 스텝에서 낮은 끝 $v_{j^\star}$로 이동하는 사건.

확률 하한: $v_{X_{fall}} = v_{i^\star}$일 확률은 $\mu_0(v_{i^\star}) \geq \mu_{min}$이고, $v_{i^\star}$에서 $v_{j^\star}$로의 직접 flow 확률은 $P_{RW}(v_{i^\star}, v_{j^\star}) > 0$이므로, 단일 라운드에서 $p_0 := \mu_0(v_{i^\star}) \cdot P_{RW}(v_{i^\star}, v_{j^\star}) > 0$이다. 

$k$개 이상의 라운드에서 큰 낙차 사건이 최소 1회 발생할 확률: $q_0 := 1 - (1-p_0)^k > 0$.

큰 낙차 라운드의 flow 개선: $E_r$ 발생 시, 스텝 $j = 0$에서 $\hbar(X_0, t_0) = \hbar(i^\star, t_0)$, 스텝 $j \geq 1$에서 $\hbar(X_j, t_0{+}j) \leq \hbar(j^\star, t_0) + (j-1)b \leq \hbar(i^\star, t_0) - M(t)/(n-1) + jb$. 따라서:
...
**$k$ 라운드 동안 경사 간선의 안정성.** 위의 개선량은 $M(t)/(n-1)$에 비례한다. 그런데 $M(t)$는 $k$ 라운드 동안 변할 수 있으므로, 이 하한이 유지되는지 확인해야 한다. $M(t)$가 줄어드는 유일한 경로는 $+b$가 현재 최저점 노드에 쌓여 $\min$이 올라가는 것인데, 한 스텝에 $+b$는 하나의 노드에만 추가되므로 $\min$은 최대 $b$만큼만 상승할 수 있다. 따라서 $s$스텝 후:

$$M(t+s) \geq M(t) - sb$$

$k$ 라운드는 총 $T := k(T_{max}+1)$스텝 이하이므로, $M$의 총 감소량은 $Tb$ 이하이다. $M(t) > 2Tb$인 영역에서는:

$$M(t+s) \geq M(t) - Tb > \frac{M(t)}{2}$$

즉, $k$ 라운드 내내 $M(t+s) > M(t)/2$이므로, 개선량의 하한 $M(t)/(2(n-1))$이 유지된다.

::: {.callout-note collapse="true" title="왜 $M(t) > 2Tb$만 따지면 충분한가?"}
최종 목표는 Foster-Lyapunov 기준을 적용하여 $\Phi(t)$의 recurrence를 보이는 것이다. 이 기준은 어떤 유한 집합 바깥에서만 drift가 음수이면 충분하므로, $M(t) \leq 2Tb$인 (유한한) 영역은 분석에서 제외해도 된다.
:::
...
**(I) $\mu_0$-편향 상한.** $\alpha := \|\boldsymbol{\mu}_0 - \mathbf{u}\|_1$로 놓자. 각 라운드의 기여 $\leq m_r \hbar(X_{fall,r}, t_0) + C_{round}$이고, $\mathbb{E}[\hbar(X_{fall}, t_0)] = \sum_i (\mu_0(i) - 1/n) \hbar(i, t_0) \leq \alpha M(t)/2$이므로:
...
$$\boxed{\overline{\operatorname{SD}}^2_{ij}(t) = \frac{1}{t}\sum_{s=0}^{t} (\delta_i(s) - \delta_j(s))^2 \;\xrightarrow{t \to \infty}\; \mathbb{E}_\pi[(\delta_i - \delta_j)^2] =: c_{ij} \quad \text{a.s.}}$$
