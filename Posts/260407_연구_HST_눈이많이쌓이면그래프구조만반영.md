...
| $q_0 := kp_0$ | $T$ 스텝 내 완전 라운드 $\geq k$개에서의 big-drop **기대 개수의 하한** |
...
여기서 $k = \lceil 1/\mu_{min} \rceil$, $\alpha = \|\boldsymbol{\mu}_0 - \mathbf{u}\|_1$ (차수 비균등도), $q_0 := kp_0$ (big-drop 기대 개수의 하한), $\mu_{min} = \min_i \mu_0(v_i)$이다. 특히 **정칙(regular) 그래프에서는 $\alpha = 0$이므로 (DC)가 자동으로 성립**한다.
...
> **Figure.** $v_{i^\star}$에 눈이 떨어지고 첫 flow 스텝에서 $v_{j^\star}$로 이동하면 $M$에 비례한 개선 발생. 단일 라운드 big drop 확률 하한 $p_0 := \mu_{min} \cdot P_{RW,\min} > 0$, $k$ 라운드의 기대 big-drop 횟수 하한 $q_0 := kp_0 > 0$.
...
$k$개의 완전 라운드에서 각 라운드의 big-drop 확률 $\geq p_0$이므로, **기대 big-drop 횟수 $\mathbb{E}[N] = \sum_{r=1}^k \Pr(E_r) \geq kp_0 =: q_0$** (선형성, 라운드간 독립성 불필요).
...
1. (**확률**) window 내 각 **완전** 라운드에서 큰 낙차 확률이 $\geq p_0$로 유지되므로, $k$개 이상의 완전 라운드의 기대 big-drop 횟수 $\geq kp_0 = q_0$.
...
**Big-drop 이벤트 확률과 기여.** $T = (k+1)(T_{max}+1)$의 버퍼 덕분에 window 내 **완전** 라운드는 최소 $k$개 보장. 각 라운드는 독립적으로 big-drop일 수 있으니, **window 내 big-drop 발생 횟수 $N := \sum_{r=1}^k \mathbf{1}_{E_r}$는 $0, 1, \ldots, k$의 확률변수**.

big-drop 라운드들의 음의 기여를 indicator로 표현:

$$S_{\text{big}} \;:=\; \sum_{r=1}^{k} \mathbf{1}_{E_r} \sum_{j=1}^{m_r-1}\big(\hbar(\tilde X_j, t_r{+}j) - \hbar(\tilde X_0, t_r)\big)$$

여기서 $\mathbf{1}_{E_r}$는 round $r$이 big-drop인지 여부를 나타내는 확률변수. 한 big-drop 라운드 ($\mathbf{1}_{E_r}=1$)의 기여는 $\leq -M(t_r)/(n-1) + C_{round}$이고, $(\heartsuit)$로 $M(t_r) \geq M(t) - Tb$ 환산:

$$S_{\text{big}} \;\leq\; \sum_{r=1}^k \mathbf{1}_{E_r}\left[-\frac{M(t)}{n-1} + \frac{Tb}{n-1} + C_{round}\right]$$

기댓값 취하기:

$$\mathbb{E}[S_{\text{big}}] \;\leq\; \mathbb{E}[N]\left[-\frac{M(t)}{n-1} + \frac{Tb}{n-1} + C_{round}\right]$$

여기서 **선형성 + 각 $\Pr(E_r) \geq p_0$**로:

$$\mathbb{E}[N] = \sum_{r=1}^k \Pr(E_r) \;\geq\; kp_0 \;=:\; q_0$$

(라운드간 독립성 불필요, 단순 선형성.) 따라서:

$$\mathbb{E}[S_{\text{big}}] \;\leq\; q_0 \cdot \left[-\frac{M(t)}{n-1} + \frac{Tb}{n-1} + C_{round}\right] \;=\; -q_0\cdot\frac{M(t)}{n-1} + D_{II}$$

여기서 $D_{II} := q_0(Tb/(n-1) + C_{round})$.
...
> **Remark.** 이 bound는 $\mathbb{E}[N] \geq kp_0$를 사용한 결과로, 라운드간 독립성 가정이 불필요. 실제 $\mathbb{E}[N]$이 $kp_0$보다 더 큰 그래프에선 더 tight한 bound 가능. (참고로 "$k$ 라운드 중 1회 이상 발생할 확률"은 $1-(1-p_0)^k$이며 Bernoulli로 $\leq kp_0$이라 덜 tight.)
