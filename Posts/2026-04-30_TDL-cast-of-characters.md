...
**김K**: 지난 편 보고 댓글이 하나 왔어요. "진영은 알겠는데, 그래서 누가 그 진영의 두목인지 모르겠다."

**박P**: 좋은 지적이에요. 오늘은 **이 분야의 캐릭터 시트**를 만들어드릴게요. 등장인물 9팀.

---

## 👑 1. Bianconi 학파 — "Dirac operator를 들고 온 사람들"

**박P**: 본거지는 **Queen Mary University of London**. 두목은 **Ginestra Bianconi**.

**김K**: 이 사람 원래 통계물리 출신 아니에요?

**박P**: 맞아요. 복잡계 네트워크의 거장. 그런데 최근 **Dirac operator를 graph/hypergraph에 갖다 붙이는 작업**으로 폭발 중. 본 데이터셋 대표작이 directed hypergraph + Dirac operator로 ABIDE 자폐 데이터를 풀어낸 논문 [1]. 톤이 거의 **Nature Communications**급.
...
**박P**: **Sapienza University of Rome**. 두목은 **Sergio Barbarossa**, 부두목은 **Stefania Sardellitti**.

**김K**: Barbarossa는 신호처리 학계의 거장이죠.

**박P**: 그렇죠. 이 사람들이 **TSP(Topological Signal Processing)** framework를 만들었어요. 단순 그래프 위 신호처리(GSP)를 simplicial complex / cell complex로 확장한 것.
...
**김K**: 이 사람들 진짜 multitasking이네요.

**박P**: 동시에 4-5개 라인을 굴립니다. 거의 **모든 핵심 논문에 IEEE 저널 톤**.

---

## 🇬🇧 3. Cambridge 학파 — "Sheaf와 simplicial attention"

**박P**: **University of Cambridge**. 두목은 **Pietro Liò**.

**김K**: Liò는 진짜 어디에나 있는 사람이에요.

**박P**: 양적으로 압도적. 우리 데이터셋만 봐도 sheaf, simplicial, hypergraph 거의 모든 클러스터에 끼어 있어요.
...
**김K**: 마지막 두 편은 홍콩-항저우-싱가포르-케임브리지 합작이네요.

**박P**: 네. **이게 최근 1년의 가장 강력한 hypergraph 동맹**입니다.

---

## 🦁 4. Oxford 학파 — "Bronstein의 GFM 진영"

**박P**: **University of Oxford**. 두목은 **Michael Bronstein**, 보좌는 **Xiaowen Dong**.

**김K**: Bronstein은 Geometric Deep Learning 책의 그 사람이죠.

**박P**: 맞아요. 그런데 최근 행보가 흥미로워요. **Graph Foundation Model이 안 된다는 걸 본인이 직접 증명함** [9]. 결론: 고정 backbone GFM은 task-dependent architectural attribute에 robust하지 않다.

**김K**: 다음 단계는 architecture-adaptive GFM이라는 거죠.

**박P**: 이게 시드 토픽이에요. 6개월 안에 후속 논문 떼거지로 나올 겁니다.

---

## ⚖️ 5. EPFL / Aachen 학파 — "스펙트럴-메시지패싱 종전 협정파"

**박P**: 두 진영을 통합한 **종전 협정문**을 쓴 사람들 [10]. **Frossard, Vandergheynst, Schaub, Morris, Wolf, Levie** — 양 학파 거장 다 모았어요.

**김K**: 진짜 어벤져스네요.

**박P**: 이 논문 한 편으로 **GSP 학파와 MPNN 학파가 같은 framework로 묶입니다.** 박사 1년차한테 던져주기 좋은 핵심 레퍼런스.

---

## 🇰🇷 6. 한국 분파 — "Yonsei 라인"

**박P**: 한국에서도 한 팀이 들어와 있어요.

대표작 [11]: **"Are Graph Transformers Necessary?"** — Graph Transformer 없이 MPNN + fractal node로 long-range 처리.

**김K**: 이거 도발적인 제목이네요.

**박P**: 의도된 도발이죠. **"Graph Transformer는 필요 없다"** 가 본문 주장이에요. 또 다른 한국 그룹은 고전 TDA 쪽 [12] — RCLA 알고리즘으로 **Poisson noise 모델 하 stability theorem** 증명.

---

## 🧠 7. Padova 학파 — "이론신경과학의 부활"

**박P**: **University of Padova (이탈리아)**. 두목은 **Samir Suweis**.

대표작 [13]: heterogeneous DMFT(동적 평균장 이론)로 **MICrONS connectome(쥐 시각피질 1mm³ 전체 시냅스)** 검증.

**김K**: q-bio 카테고리네요. 이거 PRL/PNAS 노리는 톤?

**박P**: PRX급. 이론신경과학에서 굉장히 무거운 논문이에요.
...
**김K**: Mozer는 옛날 RNN 시절 거장이죠.

**박P**: **연어 회귀**. 옛날 RNN 학파가 위상학으로 무장하고 돌아온 셈이에요.
...
**김K**: 이거 mechanistic interpretability하고 합쳐지면 무서워지겠네요.
...
**김K**: 이거 보니까 **이탈리아 라인이 진짜 무섭네요.** Bianconi-Sapienza-Padova가 한 줄로 서면.

**박P**: 정확해요. **이탈리아가 TDL의 실리콘밸리**예요. 다음 편(시리즈 3편)에서 그들이 휘두르는 **수학적 무기들**을 정리해드릴게요.
