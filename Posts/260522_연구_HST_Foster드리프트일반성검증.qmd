...
수열 $\{M(S_{t_r})\}_r$ 자체의 성질이 궁금하다. 독립인가, correlated 인가, mixing 인가, 확률적 유계 ($O_p(1)$) 인가, tight 인가? 대표 4 그래프 (bounded 2 — $K_{10}$, $C_{20}$ + 발산 2 — kite, Helm Bidir) 에 두 진단을 적용한다.

![](attachments/260522_31c9e5_03.png)

sample autocorrelation $\rho(k) := \mathrm{Corr}(M_r, M_{r+k})$ — i.i.d. 이면 lag 1 부터 $\pm 1.96/\sqrt{n}$ 신뢰구간 안. $K_{10}$ 은 lag 1 에서 $\rho \approx 0.92$ 로 강하게 양상관이지만 lag 100 부근부터 0 으로 수렴 — short-range correlated, 결국 mixing. $C_{20}$ 은 lag 100 에서도 $\rho \approx 0.55$ 로 mixing 이 느리다. 발산 두 그래프는 lag 200 까지도 $\rho \approx 1$ — 거의 deterministic trajectory 처럼 round 따라 monotone 증가, 본질적으로 mixing 이 없는 상태.

![](attachments/260522_31c9e5_04.png)

각 그래프의 round 구간 100 bin 으로 나눠 bin 별 5/50/95 quantile band + running max. bounded class ($K_{10}$, $C_{20}$) 는 모든 quantile 이 후미에서 plateau, running max 도 빠르게 안정 — marginal 분포가 stationary 라 $\sup_r \mathbb{P}(M_r > K) \to 0$ as $K \to \infty$, 즉 tight ($O_p(1)$). 발산 class (kite, Helm Bidir) 는 median 부터 95% quantile 까지 모두 round 따라 선형 증가, running max 도 끊임없이 자란다 — marginal 분포가 round 따라 right-shift, tight 도 $O_p(1)$ 도 아니고 $M_r = \Theta_p(r)$.

| 그래프 | lag-1 ACF | lag-100 ACF | tight? | $M_r$ 의 order |
|---|---|---|---|---|
| $K_{10}$ | $0.92$ | $-0.01$ | yes | $O_p(1)$ |
| $C_{20}$ | $0.99$ | $0.55$ | borderline | $O_p(1)$ likely |
| kite | $1.00$ | $1.00$ | no | $\Theta_p(r)$ |
| Helm Bidir | $1.00$ | $1.00$ | no | $\Theta_p(r)$ |

질문에 답을 정리하면: $\{M_r\}$ 은 **모든 그래프에서 독립이 아니다** (lag-1 ACF $\ge 0.92$). bounded class 에서는 short-range correlated + mixing (Markov-like), 발산 class 에서는 ACF 가 lag 100 까지 1 부근으로 본질적으로 deterministic-like trajectory. **$O_p(1)$ / tight 여부는 그래프 클래스에 따라 갈린다** — bounded class 만 tight, 발산 class 는 $r$ 의 함수로 자라 tight 가 아니다.
