...
질문별 답 (그룹 A 한정; 그룹 C 는 시계열 figure 가 그대로 답 — mixing X, not tight, $M_r = \Theta_p(r)$):

| 질문 | 그룹 A 답 |
|---|---|
| 독립? | ✗ (lag-1 ACF $\in [0.92, 0.999]$) |
| correlated? | yes |
| mixing? | 그래프별 차이 — $K_{10}$ 만 빠름 (lag 100 에서 $\rho \approx 0$), 나머지 long-range correlated |
| $O_p(1)$ / tight? | ✓ tight (marginal stationary, running max 안정) |
