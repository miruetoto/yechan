...
질문에 답을 정리하면: 그룹 A 안에서 $\{M_r\}$ 은 **모두 강하게 correlated** (lag-1 ACF $\ge 0.92$, 독립 아님), mixing 속도는 그래프별 차이가 크지만 ($K_{10}$ 빠르게 mixing, 나머지 long-range correlated) 결국 모두 stationary 분포 위 — **tight, $O_p(1)$**. 그룹 C 는 본 진단 적용 대상 밖이고 시계열 figure 가 그대로 답이다.
