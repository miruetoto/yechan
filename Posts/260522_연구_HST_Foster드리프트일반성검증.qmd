...
그래프 클래스에 따라 $M(S_{t_r})$ 시계열이 두 종류로 명확히 나뉜다. fully symmetric (Cycle, Complete, Ring, Cylinder bidir) 은 시계열이 작은 상수 (∼ 1) 부근에서 흔들리는 bounded process — height range 가 시간이 흘러도 일정 scale 위로 안 커진다 — 이를 **그룹 A** 라 부르자. 반대로 비대칭 그래프 (kite, Helm Bidir, Star Outward, $C_{60}$+pendant 등) 는 시계열이 round 가 진행할수록 거의 선형으로 발산 — height range 가 무한히 커진다 — **그룹 C**.

수열 $\{M(S_{t_r})\}_r$ 자체의 성질이 궁금하다. 독립인가, correlated 인가, mixing 인가, 확률적 유계 ($O_p(1)$) 인가, tight 인가? 그룹 C 는 $M_r$ 자체가 round 따라 발산이라 진단의 답이 자명 (mixing X, tight X, $M_r = \Theta_p(r)$) — 흥미는 그룹 A 안에서 어떻게 갈리는지에 있다. 그룹 A 5 그래프 ($K_{10}$, $C_{20}$, Cyl Inter-bidir, Wheel Bidir, Bidir Ring) 에 두 진단을 적용한다.
...
가정의 좌변 $B(S_{t_r})$ 자체의 round-index 시계열도 같은 양식으로 본다. instantaneous 추정 $B^{\rm emp}_r := \sum_u \#_u(r) \hbar(u, t_r)$ 를 round 별 한 sample 로 기록 (다수 round 의 평균이 $B(S_{t_r})$ 의 추정).

![](attachments/260522_31c9e5_05.png)

그룹 A 5 그래프 ($K_{10}$, $C_{20}$, Cyl Inter-bidir, Wheel Bidir, Bidir Ring) 는 모두 $B^{\rm emp}$ 시계열이 0 부근에서 작은 fluctuation — tail mean $|\langle B \rangle| < 0.1$, tail std $\le 1.1$. 가정 $B \le -\varepsilon_B M + C_B$ 의 우변이 $-\varepsilon_B M$ 음수 $+ C_B$ 라 좌변이 0 부근 이면 $\varepsilon_B M$ 도 작은 정도. $M$ 도 작은 그룹 A 에서 두 항이 모두 작아 가정 trivially 만족.

그룹 C 6 그래프는 $B^{\rm emp}$ tail mean 이 양 방향으로 큰 값 — Star Outward $+4851$, kite $+345$, Helm Bidir $+219$, D-Helm Bidir $+2.4$, $C_{60}$+pendant $+0.48$, Bidir Path $+0.07$. 음수가 아니라 **큰 양수** 라는 점이 직접 가정 위반이다 — 가정 우변 $-\varepsilon_B M + C_B$ 가 음 방향으로 작용해야 하는데 좌변이 양 방향으로 크다. 즉 walker 가 height 높은 노드를 자주 visit (visit count 와 ħ 가 양 상관) — mean-reversion 의 반대.

추가로 가정의 정량 형태도 raw scatter 로 확인할 수 있다. 각 round 의 $(M_r, B^{\rm emp}_r)$ scatter 에 OLS $B = -\varepsilon_B M + C_B$ 회귀를 fit 한 결과:
...
결론적으로 Assumption 1 은 ABC 의 핵심 hypothesis 임에도 모든 그래프에서 자동 성립하는 보편 명제가 아니다. Theorem A (balanced) 의 적용 범위는 사실상 그룹 A (fully symmetric 또는 거의 대칭) 로 제한될 가능성이 있고, 그룹 C (Star Outward, Helm 류, kite) 에서는 Foster-Lyapunov 가 직접 적용되지 않을 수 있다.
...
시뮬 코드와 figure 생성 코드는 본인 책상에 보관: `260522_소미_foster_verify.py` (s181 sequential 약 60초), `260522_소미_foster_figs.py` (B-M scatter), `260522_소미_foster_M_series.py` ($M$ 시계열), `260522_소미_foster_M_diag.py` (ACF + tightness). raw 캐시: `results/numeric/260522_소미_foster_verify.py/Guebins-Mac-mini.local_foster_verify.npz`.
