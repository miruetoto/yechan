...
| 질문 | 그룹 A | 그룹 C |
|---|---|---|
| 독립? | ✗ (lag-1 ACF $\in [0.92, 0.999]$) | ✗ (lag 200 까지 ACF $\approx 1$) |
| correlated? | yes | yes (거의 deterministic-like) |
| mixing? | 그래프별 차이 — $K_{10}$ 만 빠름 (lag 100 에서 $\rho \approx 0$), 나머지 long-range correlated | mixing 없음 — $M_r$ 자체가 round 따라 monotone 증가 |
| $O_p(1)$ / tight? | ✓ tight (marginal stationary, running max 안정) | ✗ not tight, $M_r = \Theta_p(r)$ |
