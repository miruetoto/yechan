...
date: 04/26/2026
draft: false
output-file: 260426_cdbaf0.html
---
---
...
여기에서 $t_r$을 임의의 시점 $t$로 보정하면 $-\frac{M(t_r)}{n-1} \leq -\frac{M(t)}{n-1}+\frac{Tb}{n-1}$ 이 된다. T시간동안 몇개의 라운드에서나 big-drop이 일어날까? 다른건 다 빼고 그 빅드랍이 $i^\star$에 떨어져서 바로 $j^\star$로 가는 경우만 잡아도 라운드별로 $p_0$ 이다. $k$의 라운드가 최소한 완전하게 있으니까 기대값의 센스에서 $kp_0$번정도 그러한 라운드가 포함될 것이라 기대할 수 있다. 따라서 