...
| $h_i := h(v_i, t) - \bar{h}(t)$ | 중심화 높이 (증명 내 약어) |
| $\Phi(t) := \sum_{i=1}^{n}(h(v_i,t) - \bar{h}(t))^2$ | Lyapunov 함수 (높이 분산, 포텐셜) |
| $M(t) := \max_i h_i(t) - \min_i h_i(t)$ | 높이 범위 (최고−최저) |
...
| $v^+,\, v^-$ | 최고점/최저점 노드: $\arg\max_i h_i$, $\arg\min_i h_i$ |
...
**Round의 정의.** 이하의 증명에서 **라운드(round)** $r$을 하나의 완전한 눈 생애 주기로 정의한다:

$$\text{round } r: \quad X_{fall}(r) \sim \boldsymbol{\mu}_0 \;\xrightarrow{+b}\; \text{flow chain} \;\to\; X^*(r) \text{에 정착}$$

즉, 새 눈이 $\mu_0$에 따라 낙하하고, flow를 거쳐 최종 정착 노드 $X^*(r)$에 $+b$가 쌓이는 것까지가 하나의 라운드이다. flow 중에는 $+b$가 이동만 하므로, 한 라운드의 **순효과(net effect)**는 $X^*(r)$에 $+b$ 하나이다. 한 라운드는 마르코프 체인의 최대 $T_{max}+1$ 스텝에 해당한다.

이하 Step 2의 drift 분석은 이 라운드를 시간 단위로 사용한다. $\Phi(r)$, $\boldsymbol{\delta}(r)$ 등은 라운드 $r$ 시작 시점(= 새 눈 낙하 직전)의 값을 의미한다.
...
**항 1: $2b(h_\ell - \bar{h})$.** $v_\ell$이 이미 평균보다 높으면 ($h_\ell > \bar{h}$) 눈이 쌓여서 더 높아지니까 분산이 **증가**한다. 반대로 평균보다 낮은 노드에 쌓이면 ($h_\ell < \bar{h}$) 평균 쪽으로 당겨지니까 분산이 **감소**한다. 즉, 낮은 곳에 눈이 쌓일수록 $\Phi$가 줄어든다 — 이것이 flow가 $\Phi$를 줄이는 핵심 메커니즘이다.
...
**1-round $\Phi$ 변화.** $X^*(r+1)$을 라운드 $r+1$에서 눈이 최종 쌓이는 노드라 하면:

$$\mathbb{E}[\Phi(r+1) - \Phi(r) \mid \boldsymbol{\delta}(r)] = 2b \cdot \mathbb{E}\big[h_{X^*(r+1)} \mid \boldsymbol{\delta}(r)\big] + \frac{(n-1)b^2}{n} \tag{$\dagger$}$$

::: {.callout-tip collapse="true" title="($\\dagger$) 직관: 이 식이 말하는 것"}

Lemma 1에서 $v_\ell$은 "눈이 실제로 쌓이는 노드"였다. 그런데 HST에서는 눈이 어디에 쌓일지가 확률적이다 — 떨어진 뒤 flow를 타고 이동해서 최종 정착 노드 $X^*(r+1)$에 쌓인다. 그래서 Lemma 1의 $h_\ell$을 $h_{X^*(r+1)}$로 바꾸고 기대값을 씌운 것이 ($\dagger$)이다.

이 식의 핵심은 $\mathbb{E}[h_{X^*(r+1)} \mid \boldsymbol{\delta}(r)]$, 즉 **"눈이 최종적으로 쌓이는 노드의 중심화 높이의 기대값"**이다. 이 값이 음수면 — 평균적으로 낮은 곳에 눈이 쌓인다면 — 첫째 항이 음수가 되어 $\Phi$가 감소한다. Flow는 높은 곳에서 낮은 곳으로 눈을 밀어내므로, flow가 잘 작동하면 $\mathbb{E}[h_{X^*(r+1)}] < 0$이 되어 분산이 줄어든다.

$\frac{(n-1)b^2}{n}$ 양의 보정 항이 항상 붙지만, $\Phi$가 제곱합이므로 높이차가 충분히 크면 $|2b \cdot h_{X^*(r+1)}|$이 이 보정을 압도한다 (아래 수치 예시 참조).
:::
...
둘 다 낮은 쪽에 $b=1$이 쌓인 동일한 사건인데, Case A에서는 $\Phi$가 $99.50$ 줄고 Case B에서는 $9.50$밖에 안 줄었다. Lemma 1의 공식으로 검산하면 $\Delta\Phi = 2b \cdot h_\ell + \frac{(n-1)b^2}{n} = 2(1)(-50) + \frac{1}{2} = -99.50$ (Case A), $2(1)(-5) + \frac{1}{2} = -9.50$ (Case B)으로 정확히 일치한다.

핵심: **같은 $b$가 쌓여도**, $h_\ell$이 $-50$이냐 $-5$이냐에 따라 $\Phi$ 감소량이 **10배** 차이난다. 이것이 "$\Phi$가 제곱합이라서 높이차가 클수록 음의 drift가 강해지는" 메커니즘이다.
:::

**$k$-round drift로의 전환.** 1-round drift만으로는 모든 그래프 구조에서 음의 drift를 보장할 수 없다. 다음의 비대칭 바벨 그래프가 대표적 반례이다.
...
1-round의 문제를 보자. 알고리즘의 flow 규칙은 $y(v_i) \geq y(v_j)$이면 flow가 발생하는 것이므로 (등호 포함), 높이가 같아도 눈이 이동한다. 눈이 $K_4$ 내부의 노드 $v_i$에 떨어지면 ($\mu_0(v_i)$가 큼), $v_i$의 높이가 $b$만큼 올라가고 flow가 인접 노드로 발생하는데, 이 인접 노드 역시 대부분 $K_4$ 내부이다. 즉, **눈이 $K_4$ 안에서 떨어지고 $K_4$ 안에서 흡수되는** 경우가 지배적이다. Bridge를 통해 $K_2$ 쪽으로 flow가 흘러가려면 $v_4$에 눈이 떨어지고 $v_5$가 $v_4$보다 높지 않아야 ($y(v_4) \geq y(v_5)$) 하는데, $\mu_0$ 편향으로 $K_4$ 쪽이 이미 높아져 있으므로 이 사건의 확률은 상대적으로 작다.

결과적으로 1-round에서는 $\mu_0$의 편향이 flow의 균등화 효과를 압도하여, $K_4$ 쪽에 눈이 편중 축적되고 $\Phi$가 **증가**하는 방향의 drift가 발생할 수 있다. 그러나 $k$ 라운드에 걸쳐 보면, 높이차가 충분히 커진 상태에서는 경사 간선 $(v_4, v_5)$을 통한 flow가 반드시 발생하여 큰 낙차를 만들고, 이 음의 기여가 $\mu_0$ 편향의 양의 기여를 압도한다.

이 관찰이 $k$-round drift를 사용하는 근본적 이유이다. 구체적으로, $k$-round skeleton 체인에 **Foster-Lyapunov 기준**을 적용하여 양의 재귀성을 이끌어낸다.
...
**$k$ 라운드 동안의 $\Phi$ 변화.** $(\dagger)$를 $k$ 라운드에 걸쳐 축적하면:

$$\Phi(r+k) - \Phi(r) = \sum_{s=0}^{k-1} \Big[2b\, h_{X^*(r+s+1)}(r+s) + \frac{(n-1)b^2}{n}\Big]$$

여기서 $h_{X^*(r+s+1)}(r+s) := h(v_{X^*(r+s+1)}, r+s) - \bar{h}(r+s)$이다. 따라서:

$$\mathbb{E}[\Phi(r+k) - \Phi(r) \mid \boldsymbol{\delta}(r)] = 2b \sum_{s=0}^{k-1} \mathbb{E}[h_{X^*(r+s+1)}(r+s) \mid \boldsymbol{\delta}(r)] + k \cdot \frac{(n-1)b^2}{n} \tag{$\dagger\dagger$}$$

::: {.callout-tip collapse="true" title="($\\dagger\\dagger$) 직관: $k$번의 눈 쌓기를 합산한 것"}

($\dagger$)는 "1라운드에서 $\Phi$가 얼마나 변하는가"였다. ($\dagger\dagger$)는 그것을 $k$번 더한 것이다.

$\sum$ 안의 $\mathbb{E}[h_{X^*(r+s+1)}(r+s) \mid \boldsymbol{\delta}(r)]$는 "$s$번째 라운드에서 눈이 최종 쌓이는 노드의 중심화 높이"이다. 이 $k$개의 항 중 **대부분은 양수일 수 있다** — $\mu_0$ 편향 때문에 높은 쪽에 눈이 쌓이는 라운드가 더 많을 수 있기 때문이다. 하지만 $k$번 중 **단 한 번이라도** 눈이 아주 낮은 곳에 쌓이는 사건이 발생하면, 그 한 번의 큰 음의 $h_{X^*}$가 나머지 $k-1$번의 양의 기여와 보정 항 $k \cdot \frac{(n-1)b^2}{n}$을 모두 상쇄하고도 남는다. 이 "단 한 번의 큰 낙차 사건"이 존재함을 보이는 것이 바로 아래 핵심 사건의 구성이다.
:::

음의 drift를 확보하기 위해, $k$ 라운드 중 최소 한 번은 눈이 **높이 범위 $M$에 비례하는 큰 낙차를 발생시키는 사건**이 일어남을 보인다.

**핵심 사건의 구성.** (아래 그림 참조)

![경사 간선과 큰 낙차 사건](attachments/260407_%EC%97%B0%EA%B5%AC_HST_%EB%88%88%EC%9D%B4%EB%A7%8E%EC%9D%B4%EC%8C%93%EC%9D%B4%EB%A9%B4%EA%B7%B8%EB%9E%98%ED%94%84%EA%B5%AC%EC%A1%B0%EB%A7%8C%EB%B0%98%EC%98%81_backup_0414_01.svg)

> **Figure.** $v^+$에서 $v^-$로의 경로 위에서 비둘기집 원리에 의해 높이차가 $M(t)/(n-1)$ 이상인 경사 간선 $(u, w)$가 존재한다. 눈이 $u$에 떨어지면($\mu_0(u)$ 확률) flow를 통해 $w$ 이하로 내려간다.

$\mathcal{G}$가 연결이므로, 최고점 $v^+ = \arg\max_i h_i(t)$에서 최저점 $v^- = \arg\min_i h_i(t)$로의 경로 (이하 $h_i(t)$는 중심화 높이 $h(v_i,t) - \bar{h}(t)$; 차이값은 절대 높이를 써도 동일) $v^+ = w_0, w_1, \ldots, w_m = v^-$ ($m \leq n-1$)가 존재한다. 경로를 따른 총 높이 낙차는:

$$M(t) = h(w_0) - h(w_m) = \sum_{i=0}^{m-1}\big(h(w_i) - h(w_{i+1})\big)$$

총합이 $M(t)$이고 항의 개수가 $m \leq n-1$이므로, 비둘기집 원리(pigeonhole principle)에 의해 평균 이상의 낙차를 가진 간선이 최소 하나 존재한다: 어떤 $i$에서 $h(w_i) - h(w_{i+1}) \geq M(t)/(n-1)$. 이 간선 $(w_i, w_{i+1})$을 **경사 간선(steep edge)** $(u, w)$ ($h_u > h_w$)이라 한다.
...
**Lemma 1a ($k$-round Drift).** $M(r) := \max_i h_i(r) - \min_i h_i(r)$라 할 때, 양의 상수 $\gamma = \gamma(\mathcal{G}) > 0$이 존재하여: $M(r)$가 충분히 크면 (즉, $\Phi(r) > C_0$ for some $C_0$이면),

$$\mathbb{E}[\Phi(r+k) - \Phi(r) \mid \boldsymbol{\delta}(r)] \leq -\gamma \cdot b \cdot M(r) + k \cdot \frac{(n-1)b^2}{n}$$

::: {.callout-tip collapse="true" title="Lemma 1a 직관: 이 부등식이 말하는 것"}

이 부등식의 우변에는 두 항이 경쟁하고 있다.

- $-\gamma \cdot b \cdot M(r)$: **음의 항**. ($\dagger\dagger$)의 합 $2b\sum \mathbb{E}[h_{X^*(r+s+1)}]$을 상한한 것이다. 이 합에는 양의 항도 음의 항도 섞여 있고, $\mu_0$ 편향 때문에 $k$개 중 대부분의 라운드에서 양의 $h_{X^*}$가 나올 수 있다. 그러나 $k$ 라운드 중 단 한 번이라도 "큰 낙차 사건"이 터지면 $M(r)$에 비례하는 큰 음의 기여가 발생하여, 합 전체가 $-\gamma \cdot b \cdot M(r)$ 이하로 눌린다.
- $k \cdot \frac{(n-1)b^2}{n}$: **양의 항**. 매 라운드마다 무조건 붙는 보정 $\frac{(n-1)b^2}{n}$이 $k$번 쌓인 것이다. 이 값은 그래프가 정해지면 **상수**이다.

$M(r)$가 커지면 음의 항은 $M(r)$에 비례해서 커지지만, 양의 항은 상수로 고정이다. 따라서 $M(r)$가 충분히 크면 음의 항이 반드시 이긴다. 이것이 "$\Phi$가 무한히 커질 수 없다"는 핵심 논거이다.
:::

**증명.** $k$ 라운드 동안 눈이 최종 정착하는 위치의 (중심화) 높이 합을 분석한다.

**Step A: 큰 낙차 사건(Big Drop Event)의 확률 하한.**

라운드 $r+s+1$에서 "큰 낙차 사건" $E_s$를 다음과 같이 정의한다: 눈의 초기 낙하 지점 $X_{fall}$이 현재 경사 간선의 높은 끝 $u$에 떨어져, flow를 통해 높이가 최소 $M(r)/(2(n-1))$만큼 하강하는 사건.

![큰 낙차 사건 $E_s$의 구조](attachments/260407_03.svg)

임의의 단일 라운드에서 $E_s$가 발생할 확률을 하한한다. $X_{fall} = u$일 확률은 $\mu_0(u) \geq \mu_{min}$이다. $u$에 떨어진 눈은 더 낮은 이웃 $w$ ($h_w \leq h_u - M(r)/(n-1)$)로 flow할 수 있다. Algorithm 1의 이웃 선택 확률 $P_{RW}(u, w) > 0$이므로, $u$에서 $w$로의 직접 flow 확률은 $\mu_0(u) \cdot P_{RW}(u, w) =: p_0 > 0$이다. 이 $p_0$는 그래프 구조에만 의존하는 양의 상수이다.

**$k$ 라운드 동안 경사 간선의 안정성:** 각 라운드에서 하나의 노드에만 $+b$가 추가되므로, $k$ 라운드 후에도 높이 범위는 $M(r+s) \geq M(r) - kb$ (최대 $kb$만큼 감소 가능)이다. $M(r)$가 $kb$보다 충분히 크면 (즉, $M(r) > 2kb$), $k$ 라운드 동안 경사 간선의 높이차는 원래의 절반 이상을 유지한다: $h_u(r+s) - h_w(r+s) \geq M(r)/(2(n-1))$.

따라서 $k$ 라운드 각각에서 큰 낙차 사건이 독립적으로 발생하지는 않지만, 각 라운드의 조건부 확률은 최소 $p_0/2 > 0$ (경사 간선이 충분한 낙차를 유지하므로)으로 하한된다.

**Step B: 큰 낙차 사건의 기여 정량화.**

큰 낙차 사건 $E_s$가 발생하면, 해당 라운드에서 눈의 최종 정착지의 중심화 높이는:

$$h_{X^*(r+s+1)}(r+s) \leq h_w(r+s) \leq h_u(r+s) - \frac{M(r)}{2(n-1)} \leq \frac{M(r+s)}{2} - \frac{M(r)}{2(n-1)}$$

큰 낙차 사건이 발생하지 않는 라운드에서는 자명한 상한 $h_{X^*}(r+s) \leq M(r+s)/2 \leq M(r)/2$를 사용한다.

**Step C: $\mu_0$-편향과 flow 개선의 분리.**

각 라운드의 $\Phi$ 변화를 두 성분으로 분해한다:

$$h_{X^*(s)} = h_{X_{fall}(s)} + \underbrace{(h_{X^*(s)} - h_{X_{fall}(s)})}_{\leq\, 0 \text{ (flow는 항상 하향)}}$$

따라서 $(\dagger\dagger)$는:

$$\mathbb{E}[\Phi(r+k) - \Phi(r) \mid \boldsymbol{\delta}(r)] = \underbrace{2b \sum_{s=0}^{k-1} \mathbb{E}[h_{X_{fall}(s)}(r+s)]}_{\text{(I): } \mu_0\text{-편향 누적}} + \underbrace{2b \sum_{s=0}^{k-1} \mathbb{E}[h_{X^*(s)} - h_{X_{fall}(s)}]}_{\text{(II): flow 개선 (}\leq 0\text{)}} + k \cdot \frac{(n-1)b^2}{n}$$

**(I)의 상한 ($\mu_0$-편향).** $\alpha := \|\boldsymbol{\mu}_0 - \mathbf{u}\|_1$로 놓자 ($\mathbf{u}$는 균등 분포). 라운드 $r+s$에서:

$$\mathbb{E}[h_{X_{fall}(s)}(r+s) \mid \boldsymbol{\delta}(r+s)] = \sum_i \mu_0(v_i)\, h_i(r+s) = \sum_i (\mu_0(v_i) - \tfrac{1}{n})\, h_i(r+s) \leq \alpha \cdot M(r+s)$$

$k$ 라운드 동안 각 라운드마다 하나의 노드에만 $+b$가 추가되므로 $M(r+s) \leq M(r) + sb$이다. 따라서:

$$\text{(I)} \leq 2b \sum_{s=0}^{k-1} \alpha(M(r) + sb) = 2b\alpha k M(r) + \alpha k(k-1)b^2$$

**(II)의 하한 (flow 개선).** 큰 낙차 사건 $E_s$에서 눈은 $X_{fall} = u$에서 $w$로 flow하여:

$$h_{X^*(s)} - h_{X_{fall}(s)} \leq h_w(r+s) - h_u(r+s) \leq -\frac{M(r)}{2(n-1)}$$

$k$ 라운드 중 큰 낙차 사건이 최소 1회 발생할 확률은 $q_0 := 1 - (1-p_0/2)^k > 0$이고, 비큰낙차 라운드에서도 $h_{X^*} - h_{X_{fall}} \leq 0$이므로:

$$\text{(II)} \leq -2b \cdot q_0 \cdot \frac{M(r)}{2(n-1)} = -\frac{b\, q_0\, M(r)}{n-1}$$

**결합.** (I) + (II) + 상수항:

$$\mathbb{E}[\Phi(r+k) - \Phi(r) \mid \boldsymbol{\delta}(r)] \leq b\, M(r) \underbrace{\left[2k\alpha - \frac{q_0}{n-1}\right]}_{=:\, \Delta_{\mathrm{drift}}} + C_3 \tag{$\star\star$}$$

여기서 $C_3 := \alpha k(k-1)b^2 + k(n-1)b^2/n$은 $M(r)$에 무관한 상수이다. $\square$

**$\Delta_{\mathrm{drift}} < 0$의 충분 조건.** $k$-round drift가 $M(r)$가 커질 때 확실히 음이 되려면:

$$\Delta_{\mathrm{drift}} = 2k\alpha - \frac{q_0}{n-1} < 0 \quad \iff \quad \alpha < \frac{q_0}{2k(n-1)} \tag{$\star\star\star$}$$

이 조건이 성립하면 $\Phi(r) \geq M(r)^2/n$에 의해, $\Phi(r) > C_1 := n\big(\frac{C_3}{|\Delta_{\mathrm{drift}}| b}\big)^2$일 때 $k$-round drift가 확실히 음이 된다.

**정칙 그래프에서의 무조건적 성립.** $\mathcal{G}$가 정칙(regular)이면 $\mu_0(v_i) = 1/n$ for all $i$이므로 $\alpha = 0$이다. 따라서 $\Delta_{\mathrm{drift}} = -q_0/(n-1) < 0$이 무조건 성립하며, $k$-round drift 조건이 어떤 추가 가정 없이 증명된다.

**일반 연결 그래프로의 확장.** 비정칙 그래프에서 $\alpha > 0$이면, $(\star\star\star)$는 그래프의 차수 균등도(degree uniformity)와 flow-through 확률 $q_0$의 관계를 요구한다. 실질적으로 이 조건은 **그래프의 병목(bottleneck)이 지나치게 극단적이지 않을 때** 성립한다.
...
**양의 재귀성의 도출.** 위의 $k$-round drift 부등식 $(\star\star)$으로부터 양의 재귀성을 이끌어내기 위해, 다음의 일반적 결과를 사용한다.
...
**Proposition ($k$-round 확장).** 위 Theorem에서 1-round drift를 $k$-round drift $\mathbb{E}[\Phi(S(r+k)) - \Phi(S(r)) \mid S(r)] \leq -1$ (for $S(r) \notin \mathcal{B}$)로 바꾸어도 양의 재귀성이 성립한다.

**증명.** $S^{(k)}(m) := S(mk)$로 놓으면, Chapman-Kolmogorov 방정식에 의해 $\{S^{(k)}(m)\}$은 전이 커널 $P^k$를 가진 마르코프 체인이다 (M&T §5.5). 원래 체인의 $k$-round drift 조건은 이 skeleton 체인의 1-round drift 조건과 정확히 같으므로, 위의 Theorem을 적용하면 skeleton이 양의 재귀이다: $\mathbb{E}[\tau^{(k)}_\mathcal{B}] < \infty$ ($\tau^{(k)}_\mathcal{B} := \min\{m \geq 1 : S^{(k)}(m) \in \mathcal{B}\}$). 원래 체인의 복귀 시각 $\tau_\mathcal{B}$는 $\tau_\mathcal{B} \leq k \cdot \tau^{(k)}_\mathcal{B}$를 만족하므로 (skeleton이 시각 $m$에 $\mathcal{B}$를 방문하면 원래 체인도 시각 $mk$에 $\mathcal{B}$에 있으므로) $\mathbb{E}[\tau_\mathcal{B}] \leq k \cdot \mathbb{E}[\tau^{(k)}_\mathcal{B}] < \infty$이고, 원래 체인도 양의 재귀이다. $\square$

**Corollary (양의 재귀성).** $(\star\star\star)$이 성립하는 연결 그래프에서, $(\star\star)$의 drift를 적절히 정규화하면 $k$-round Foster-Lyapunov 조건이 만족되므로, 유한 집합 $\mathcal{B}_\Phi := \{S : \Phi \leq C_1\}$은 양의 재귀(positive recurrent)이다. $\Phi \leq C_1$이면 $M \leq \sqrt{nC_1}$이므로 높이차가 유계이다.
...
**라운드 ↔ 시간 스텝 관계.** Step 2의 drift 분석은 라운드 $r$ 단위로 수행되었다. 라운드 수 $R$과 실제 마르코프 체인 시간 스텝 $T$의 관계는 $R \leq T \leq R(T_{max}+1)$이다 (한 라운드는 최소 1스텝, 최대 $T_{max}+1$스텝). 따라서 라운드 단위의 양의 재귀성 ($\mathbb{E}[\tau^{(round)}_\mathcal{B}] < \infty$)은 시간 스텝 단위의 양의 재귀성 ($\mathbb{E}[\tau^{(step)}_\mathcal{B}] \leq (T_{max}+1)\,\mathbb{E}[\tau^{(round)}_\mathcal{B}] < \infty$)을 즉시 함의한다.

**Step 1–3의 결합:** $\{S(t)\}$는 격자 $\boldsymbol{\delta}(0) + b\mathbb{Z}^{n-1}$ 위에서 정의된 마르코프 체인이며, Step 2의 $k$-round Foster-Lyapunov 조건에 의해 유한 집합 $\mathcal{B}_\Phi$가 양의 재귀이고, Step 3에 의해 $\mathcal{B}_\Phi$ 위에서 비가약(irreducible)이다. 따라서 $\{S(t)\}$는 비가약이고 양의 재귀적인(irreducible and positive recurrent) 마르코프 체인이다.
...
$$\Phi(\boldsymbol{\delta}) = \sum_{i=1}^{n} (h_i - \bar{h})^2$$

즉 **높이의 분산**이다. (논문의 노드 집합 $V$와의 기호 충돌을 피하기 위해 $\Phi$로 표기한다.) $\Phi$는 엄밀히 $S = (\boldsymbol{\delta}, X, Z)$의 세 성분 중 $\boldsymbol{\delta}$에만 의존한다 — $h_i - \bar{h}$는 높이차 벡터 $\boldsymbol{\delta}$로부터 결정되고, 현재 flow 위치 $X$나 연속 flow 카운터 $Z$와는 무관하기 때문이다. 높이차가 클수록 $\Phi$가 크고 (불안정), 높이가 균등해질수록 $\Phi$가 작아진다 (안정). $\Phi \geq 0$이라는 성질은 "바닥이 있다"는 뜻으로, 이것이 증명의 핵심이다.
