---
title: "연구 ▷ HST ▷ SD² 가 t, t², t³ 로 스케일링되는 이유"
author: 도비
date: 05/28/2026
draft: false
output-file: 260528_03b2cb.html
fontsize: 0.85em
...
```{python}
#| label: fig-sd2
#| fig-cap: "세 경우의 SD²(t). 로그-로그 스케일에서 기울기 1, 2, 3."
...
**H.** 그리고 $\Delta h(t') = \sum_{s=1}^{t'} \xi_s$ 예요 — 증분들의 누적합. 이제 Thm B 조건에서 $i$와 $j$ 를 방문할 확률이 비슷하면, $\xi_s$ 들이 **평균 0에 가까운 i.i.d.** 처럼 행동해요. 동전 던지기요.

**G.** 동전을 $t'$ 번 던지면 앞-뒤 차이가 $\sqrt{t'}$ 만큼 퍼지는 그거요?

**H.** 바로 그거예요. CLT. 동전을 $t'$ 번 던졌을 때 앞면 횟수 - 뒷면 횟수 의 표준편차가 $\sqrt{t'}$ 인 것처럼, 높이차 $\Delta h(t')$ 도 $\sqrt{t'}$ 만큼 방랑해요. 중심이 없는 랜덤워크. 1차원 취객이 $t'$ 걸음 걸은 뒤 출발점으로부터 $\sqrt{t'}$ 거리에 있는 것과 같아요.

**G.** 그러면 Thm A 는요? 상수라고 했는데.

**H.** Thm A 는 그래프가 더 "균형잡혀" 있어서 높이차가 수렴하는 경우예요 — 시간이 지날수록 랜덤워크가 평균으로 당겨지는 (mean-reverting) 상황. Thm B 는 평균으로 돌아오지 않고 계속 방랑하는 경우고요.
