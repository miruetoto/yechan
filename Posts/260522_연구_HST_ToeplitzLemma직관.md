...
세 가중치 — 자명한 산술평균 Cesàro $a_{ns} = 1/n$, ABC Theorem~B 의 $a_{ns} = 2s/[n(n+1)]$, 그리고 일부러 (T3) 을 깨뜨린 반례 $a_{n,1} = a_{n,n} = 1/2$ — 의 모양을 직접 그려보면 다음과 같다. 각 가중치에 대해 행 $n = 10, 100, 500$ 세 개를 골라 가로축 $s$ 에 대해 plot 한 것이다.
