...
정의 2 (the chamber). 반지름 $R$의 열린 $\ell^\infty$-disk를 $\mathsf{D}_\infty(R) := \{\mathbf{d}\in\mathbb{R}^{n-1} : \|\mathbf{d}\|_\infty < R\}$로 쓴다. 각 $\mathbf{u}\in\mathbb{R}^{n-1}$을 $u_1 := 0$으로 앵커링하여 $v_1$에 상대적인 노드 높이 프로필로 읽으며, 그리하여 $u_i$는 $v_1$ 위로 잰 $v_i$의 높이이다 ($1\leq i\leq n$). chamber $\mathsf{U}$는 양의 Lebesgue 측도를 갖는 임의의 콤팩트 집합
$$
...
비고 (the constants of the chamber). 정의 2의 두 선택은 나중에 지킬 약속이다. 파라미터 $b$는 한 스텝이 쌓는 눈의 상한이다; 반지름이 왜 그것의 4분의 1인지는 뒤의 두 보조정리에서 분명해지는데, 거기서 $n$개의 낙하(각 노드에 하나씩)가 바로 그런 disk를 밀도 floor로 정확히 덮음을 보인다. chamber가 왜 원점 근처, 그리고 하나의 strict order 안(ties로부터 떨어져)에 있는지는 뒤의 한 보조정리에서 분명해지는데, 거기서 landing point에서 살아남는 gap들이 그 tie 값들로부터 고정된 여유만큼 떨어지도록 pin된다.

floor를 위해 우리는 다음의 (비현실적인) 시나리오로 전이를 아래에서 막는다. 그 다섯 항목은 window의 두 무작위성 원천을 고정하고 결과를 읽어낸다: 항목 (1)–(2)는 적재량을, 항목 (3)–(4)는 낙하들 사이에서 walker가 따라가는 경로(falls가 자유롭게 남긴 이산 선택들)를, 항목 (5)는 높이차가 어디에 놓이는지를 규정한다.
